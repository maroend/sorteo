...
public class mBoletosTransito  extends SuperModel {
	...
	public void getUsuarioSorteo(){
		...
		System.out.println("get sorteo"+sql);
	...
	public int contar(String search, int idsorteo) {
		...
		//sql += " FROM SORTEOS_BOLETOS SB, BOLETOS B WHERE SB.PK_BOLETO = B.PK1 AND SB.PK_SORTEO = '"+idsorteo+"' AND B.VENDIDO ='V' AND B.FOLIO NOT IN (SELECT FOLIO FROM BOLETOS WHERE SORTEO = '"+idsorteo+"' AND RETORNADO = 1)";
		String sql = "SELECT COUNT(B.PK1) AS 'MAX' FROM SORTEOS_BOLETOS R, BOLETOS B WHERE R.PK_BOLETO = B.PK1 AND R.PK_SORTEO = "+idsorteo+" AND " + condicion_Transito;
	...
	/**/
	public int contar(String search, long idsorteo) {
		return super.Reportes_ContarBoletosXSorteo(search, idsorteo, condicion_Transito);
	}
	public int contarXSector(String search, long idsorteo, long idsector) {
		return super.Reportes_ContarBoletosXSector(search, idsorteo, idsector, condicion_Transito);
	}
	public int contarXNicho(String search, long idsorteo, long idsector, long idnicho) {
		return super.Reportes_ContarBoletosXNicho(search, idsorteo, idsector, idnicho, condicion_Transito);
	}
	public int contarXColaborador(String search, long idsorteo, long idsector, long idnicho, long idcolaborador) {
		return super.Reportes_ContarBoletosXColaborador(search, idsorteo, idsector, idnicho, idcolaborador, condicion_Transito);
	}
	
	public ResultSet paginacion(int pg, int numreg, String search,int idsorteo) {
		/*
		String sql = "SELECT S.PK_BOLETO,B.VENDIDO,B.ABONO,B.PK1,B.FOLIO,B.COSTO, B.TALONARIO, B.FORMATO, B.SORTEO, B.PK_TALONARIO, B.RETORNADO,B.INCIDENCIA, "
			...
			+ " FROM SORTEOS_BOLETOS S, BOLETOS B WHERE S.PK_BOLETO = B.PK1 AND S.PK_SORTEO = '"+idsorteo+"' AND B.VENDIDO='V' AND B.FOLIO NOT IN (SELECT FOLIO FROM BOLETOS WHERE SORTEO = '"+idsorteo+"' AND RETORNADO = 1) ";
		//*/
		String sql = 
				"SELECT R.PK_BOLETO,B.VENDIDO,B.ABONO,B.PK1,B.FOLIO,B.COSTO, B.TALONARIO, B.FORMATO, B.SORTEO, B.PK_TALONARIO, B.RETORNADO,B.INCIDENCIA"
				+ ",(select TOP 1 VENDIDO FROM TALONARIOS WHERE PK1 = B.PK_TALONARIO) AS 'VENDIDOTALONARIO'"
				+ ",(select TOP 1 PK_SECTOR  FROM SORTEOS_SECTORES_BOLETOS       WHERE PK_BOLETO = R.PK_BOLETO) AS 'PK_SECTOR'"
			//	+ ",(select TOP 1 SECTOR     FROM SECTORES                       WHERE PK1=(select TOP 1 PK_SECTOR      from SORTEOS_SECTORES_BOLETOS       where PK_BOLETO = R.PK_BOLETO)) AS 'SECTOR'"
				+ ",(select TOP 1 PK_NICHO       from SORTEOS_NICHOS_BOLETOS where PK_BOLETO = R.PK_BOLETO) AS 'PK_NICHO'"
			//	+ ",(select TOP 1 NICHO          FROM NICHOS        WHERE PK1=(select TOP 1 PK_NICHO       from SORTEOS_NICHOS_BOLETOS         where PK_BOLETO = R.PK_BOLETO)) AS 'NICHO'"
				+ ",(select TOP 1 PK_COLABORADOR from SORTEOS_COLABORADORES_BOLETOS  where PK_BOLETO = R.PK_BOLETO) AS 'PK_COLABORADOR'"
			//	+ ",(select TOP 1 NOMBRE+' '+APATERNO  FROM COLABORADORES WHERE PK1=(select TOP 1 PK_COLABORADOR from SORTEOS_COLABORADORES_BOLETOS  where PK_BOLETO = R.PK_BOLETO)) AS 'COLABORADOR'"
				+ ",(select TOP 1 CONCAT(ISNULL(C.NOMBRE,''),' ',ISNULL(C.APATERNO,'')) from SORTEOS_COLABORADORES_BOLETOS SCB, COLABORADORES C  WHERE SCB.PK_BOLETO = R.PK_BOLETO) AS 'COLABORADOR'"
				+ "FROM SORTEOS_BOLETOS R, BOLETOS B WHERE R.PK_BOLETO = B.PK1 AND R.PK_SORTEO = "+idsorteo+" AND " + condicion_Transito;
	...
	/*
	public int contarXSector(String search, int idsorteo, String table) {
		String sql = "SELECT COUNT(B.PK1) AS 'MAX'"
				+ " FROM SORTEOS_SECTORES_BOLETOS R, BOLETOS B"
				+ " WHERE R.PK_BOLETO = B.PK1 AND R.PK_SORTEO = "+idsorteo+" AND " + condicion_Transito;
	...
	/**/
	/*
	public int contarXSector_(String search,int idsorteo,int idsector) {
		String sql = "SELECT COUNT(*) AS TOTAL ";
		sql += " FROM SORTEOS_SECTORES_BOLETOS SB, BOLETOS B WHERE SB.PK_BOLETO = B.PK1 AND SB.PK_SORTEO = '"+idsorteo+"' AND SB.PK_SECTOR = '"+idsector+"' AND B.VENDIDO='V' AND B.FOLIO NOT IN (SELECT FOLIO FROM BOLETOS WHERE SORTEO = '"+idsorteo+"' AND RETORNADO = 1)";
	...
	public ResultSet paginacionXSector(int pg, int numreg, String search,int idsorteo,int idsector)
	{
		String sql = "SELECT R.PK_BOLETO,B.VENDIDO,B.ABONO,B.PK1,B.FOLIO,B.COSTO, B.TALONARIO, B.FORMATO, B.SORTEO, B.PK_TALONARIO, B.RETORNADO, B.INCIDENCIA,"
				+ " (select TOP 1 VENDIDO FROM TALONARIOS WHERE PK1 = B.PK_TALONARIO) AS 'VENDIDOTALONARIO',"
				+ " (select TOP 1 PK_SECTOR  from SORTEOS_SECTORES_BOLETOS       where PK_BOLETO = R.PK_BOLETO) AS 'PK_SECTOR',"
				+ "(select TOP 1 SECTOR FROM SECTORES      WHERE PK1=(select TOP 1 PK_SECTOR      from SORTEOS_SECTORES_BOLETOS       where PK_BOLETO = R.PK_BOLETO)) AS 'SECTOR',"
				+ " (select TOP 1 PK_NICHO       from SORTEOS_NICHOS_BOLETOS where PK_BOLETO = R.PK_BOLETO) AS 'PK_NICHO',"
			...
			//	+ " (select TOP 1 NOMBRE+' '+APATERNO  FROM COLABORADORES WHERE PK1=(select TOP 1 PK_COLABORADOR from SORTEOS_COLABORADORES_BOLETOS  where PK_BOLETO = R.PK_BOLETO)) AS 'COLABORADOR'"
				+ " (select TOP 1 CONCAT(ISNULL(C.NOMBRE,''),' ',ISNULL(C.APATERNO,'')) from SORTEOS_COLABORADORES_BOLETOS SCB, COLABORADORES C  WHERE SCB.PK_BOLETO = R.PK_BOLETO) AS 'COLABORADOR'"
			//	+ " FROM SORTEOS_SECTORES_BOLETOS R, BOLETOS B WHERE R.PK_BOLETO = B.PK1 AND R.PK_SORTEO = '"+idsorteo+"' AND R.PK_SECTOR = "+idsector+"  AND B.VENDIDO='V' AND B.FOLIO NOT IN (SELECT FOLIO FROM BOLETOS WHERE SORTEO = '"+idsorteo+"' AND RETORNADO = 1) ";
				+ " FROM SORTEOS_SECTORES_BOLETOS R, BOLETOS B WHERE R.PK_BOLETO = B.PK1 AND R.PK_SORTEO = "+idsorteo+" AND R.PK_SECTOR = "+idsector+"  AND " + SuperModel.condicion_Transito;

		if (search != "") {
			sql += " AND ((B.FOLIO = '" + search + "') OR (B.TALONARIO = '" + search + "'))   ";
		}
		
		sql += "ORDER BY B.TALONARIO ASC ";
		sql += "OFFSET (" + (pg - 1) * numreg + ") ROWS "; // -- not sure if you
															// need -1
		sql += "FETCH NEXT " + numreg + " ROWS ONLY";
	...
	/*
	public int contarNichos(String search,int idsorteo,int idsector,int idnicho) {
		String sql = "SELECT COUNT(*) AS TOTAL ";
		sql += " FROM SORTEOS_NICHOS_BOLETOS SB, BOLETOS B WHERE SB.PK_BOLETO = B.PK1 AND SB.PK_SORTEO = '"+idsorteo+"' AND SB.PK_SECTOR = '"+idsector+"' AND SB.PK_NICHO = '"+idnicho+"'  AND B.VENDIDO='V' AND B.FOLIO NOT IN (SELECT FOLIO FROM BOLETOS WHERE SORTEO = '"+idsorteo+"' AND RETORNADO = 1) ";
        int numero = 0;
	...
	public ResultSet paginacionXNichos(int pg, int numreg, String search,int idsorteo,int idsector,int idnicho)
	{
		String sql = "SELECT R.PK_BOLETO,B.VENDIDO,B.ABONO,B.PK1,B.FOLIO,B.COSTO, B.TALONARIO, B.FORMATO, B.SORTEO, B.PK_TALONARIO,B.RETORNADO, B.INCIDENCIA,"
				+ " (select TOP 1 VENDIDO FROM TALONARIOS WHERE PK1 = B.PK_TALONARIO) AS 'VENDIDOTALONARIO',"
				+ " (select TOP 1 PK_SECTOR  from SORTEOS_SECTORES_BOLETOS       where PK_BOLETO = R.PK_BOLETO) AS 'PK_SECTOR',"
			...
			//	+ " (select TOP 1 NOMBRE+' '+APATERNO  FROM COLABORADORES WHERE PK1=(select TOP 1 PK_COLABORADOR from SORTEOS_COLABORADORES_BOLETOS  where PK_BOLETO = R.PK_BOLETO)) AS 'COLABORADOR'"
				+ " (select TOP 1 CONCAT(ISNULL(C.NOMBRE,''),' ',ISNULL(C.APATERNO,'')) from SORTEOS_COLABORADORES_BOLETOS SCB, COLABORADORES C  WHERE SCB.PK_BOLETO = R.PK_BOLETO) AS 'COLABORADOR'"
			//	+ " FROM SORTEOS_NICHOS_BOLETOS S, BOLETOS B WHERE R.PK_BOLETO = B.PK1 AND R.PK_SORTEO = '"+idsorteo+"' AND R.PK_SECTOR = "+idsector+" AND S.PK_NICHO = '"+idnicho+"'  AND B.VENDIDO='V' AND B.FOLIO NOT IN (SELECT FOLIO FROM BOLETOS WHERE SORTEO = '"+idsorteo+"' AND RETORNADO = 1)" ;
				+ " FROM SORTEOS_NICHOS_BOLETOS R, BOLETOS B WHERE R.PK_BOLETO = B.PK1 AND R.PK_SORTEO = "+idsorteo+" AND R.PK_SECTOR = "+idsector+" AND S.PK_NICHO = "+idnicho+"  AND " + SuperModel.condicion_Transito;
	...
	/*
	public int contarColaborador(String search,int idsorteo,int idsector,int idnicho,int idcolaborador) {
		
		String sql = "SELECT COUNT(*) AS TOTAL ";
		sql += " FROM SORTEOS_COLABORADORES_BOLETOS SB, BOLETOS B WHERE SB.PK_BOLETO = B.PK1 AND SB.PK_SORTEO = '"+idsorteo+"' AND SB.PK_SECTOR = '"+idsector+"' AND SB.PK_NICHO = '"+idnicho+"' AND SB.PK_COLABORADOR = '"+idcolaborador+"' AND B.VENDIDO='V' AND B.FOLIO NOT IN (SELECT FOLIO FROM BOLETOS WHERE SORTEO = '"+idsorteo+"' AND RETORNADO = 1)";
        int numero = 0;
	...
	/**/
	public ResultSet paginacionXColaborador(int pg, int numreg, String search,int idsorteo,int idsector,int idnicho,int idcolaborador)
	{
		String sql = "SELECT R.PK_BOLETO,B.VENDIDO,B.ABONO,B.PK1,B.FOLIO,B.COSTO, B.TALONARIO, B.FORMATO, B.SORTEO, B.PK_TALONARIO, B.RETORNADO,B.INCIDENCIA, "
			+ " (select TOP 1 VENDIDO FROM TALONARIOS WHERE PK1 = B.PK_TALONARIO) AS 'VENDIDOTALONARIO',"
			+ " (select TOP 1 PK_SECTOR  from SORTEOS_SECTORES_BOLETOS       where PK_BOLETO = R.PK_BOLETO) AS 'PK_SECTOR',"
			...
			+ " (select TOP 1 PK_COLABORADOR from SORTEOS_COLABORADORES_BOLETOS  where PK_BOLETO = R.PK_BOLETO) AS 'PK_COLABORADOR',"
		//	+ " (select TOP 1 NOMBRE+' '+APATERNO  FROM COLABORADORES WHERE PK1=(select TOP 1 PK_COLABORADOR from SORTEOS_COLABORADORES_BOLETOS  where PK_BOLETO = R.PK_BOLETO)) AS 'COLABORADOR'"
			+ " CONCAT(ISNULL(C.NOMBRE,''),' ',ISNULL(C.APATERNO,'')) AS 'COLABORADOR'"
		//	+ " FROM SORTEOS_COLABORADORES_BOLETOS S, BOLETOS B WHERE R.PK_BOLETO = B.PK1 AND R.PK_SORTEO = '"+idsorteo+"' AND R.PK_SECTOR = "+idsector+" AND R.PK_NICHO = '"+idnicho+"' AND R.PK_COLABORADOR = '"+idcolaborador+"' AND B.VENDIDO='V' AND B.FOLIO NOT IN (SELECT FOLIO FROM BOLETOS WHERE SORTEO = '"+idsorteo+"' AND RETORNADO = 1)" ;
			+ " FROM SORTEOS_COLABORADORES_BOLETOS R, BOLETOS B WHERE R.PK_BOLETO = B.PK1 AND R.PK_SORTEO = "+idsorteo+" AND R.PK_SECTOR = "+idsector+" AND R.PK_NICHO = "+idnicho+" AND R.PK_COLABORADOR = "+idcolaborador+" AND " + SuperModel.condicion_Transito;
	...
	public String getBoletosTalonariosColaborador(){
	 ...
	 public String getBoletosTalonariosNicho(){
		...
		//TOTAL DE RETORNADOS
		 sql = "SELECT COUNT(*) AS TOTAL FROM SORTEOS_NICHOS_BOLETOS SB, BOLETOS B WHERE SB.PK_BOLETO = B.PK1 AND SB.PK_SORTEO = '" + this.getIdSorteo() + "' AND SB.PK_SECTOR = '"+this.getIdsector()+"' AND SB.PK_NICHO = "+this.getIdnicho()+" AND B.RETORNADO = 1";
		//System.out.println("sql getBoletosTalonarios: "+sql);
		 rs = db.getDatos(sql);
		try {if (rs != null) { while(rs.next()){ total += rs.getString("TOTAL")+"|";}/*rs.close();*/}} catch (SQLException e) {e.printStackTrace();}	 
	...
	//TOTAL DE RETORNADOS
	 sql = "SELECT COUNT(*) AS TOTAL FROM SORTEOS_SECTORES_BOLETOS SB, BOLETOS B WHERE SB.PK_BOLETO = B.PK1 AND SB.PK_SORTEO = '" + this.getIdSorteo() + "' AND SB.PK_SECTOR = '"+this.getIdsector()+"' AND B.RETORNADO = 1";
	//System.out.println("sql getBoletosTalonarios: "+sql);
	 rs = db.getDatos(sql);
	 try {if (rs != null) { while(rs.next()){ total += rs.getString("TOTAL")+"|";}/*rs.close();*/}} catch (SQLException e) {e.printStackTrace();}	 
	...
	//TOTAL BOLETOS  ROBADOS
	 sql = "SELECT COUNT(*) AS TOTAL FROM BOLETOS_INCIDENCIAS WHERE INCIDENCIA = 'R'  AND PK_SORTEO = '"+this.getIdSorteo()+"'";
		...
		//TOTAL DE RETORNADOS
		 sql = "SELECT COUNT(*) AS TOTAL FROM BOLETOS WHERE SORTEO = '"+this.getIdSorteo()+"' AND RETORNADO = 1";
		 rs = db.getDatos(sql);
		 try {if (rs != null) { while(rs.next()){ total += rs.getString("TOTAL")+"|";}/*rs.close();*/}} catch (SQLException e) {e.printStackTrace();}	 
