...
public class mBoletosPerdidosColaborador extends SuperModel  {
	...
	public mBoletosPerdidosColaborador() {
	...
	public int contar(String search,int idsorteo, int idsector, int idnicho,int idcolaborador) {
		...
		sql += " FROM SORTEOS_COLABORADORES_BOLETOS SB, BOLETOS B,BOLETOS_INCIDENCIAS I WHERE SB.PK_BOLETO = B.PK1 AND B.PK1 = I.PK_BOLETO AND I.INCIDENCIA = 'E' AND SB.PK_SORTEO = '"+idsorteo+"' AND SB.PK_SECTOR = '"+idsector+"' AND SB.PK_NICHO = '"+idnicho+"'  AND SB.PK_COLABORADOR = '"+idcolaborador+"'";
	...
	public ResultSet paginacion(int pg, int numreg, String search,int idsorteo, int idsector, int idnicho,int idcolaborador) {
		...
		String sql = "SELECT S.PK_BOLETO,B.VENDIDO,B.ABONO,B.PK1,B.FOLIO,B.COSTO, B.TALONARIO, B.FORMATO, B.SORTEO, B.PK_TALONARIO, B.INCIDENCIA, "
			...
			+ "FROM SORTEOS_COLABORADORES_BOLETOS S, BOLETOS B, BOLETOS_INCIDENCIAS I WHERE S.PK_BOLETO = B.PK1 AND S.PK_BOLETO = I.PK_BOLETO AND S.PK_SORTEO = '"+idsorteo+"' AND S.PK_SECTOR = '"+idsector+"'  AND S.PK_NICHO = '"+idnicho+"'  AND S.PK_COLABORADOR = '"+idcolaborador+"' AND I.INCIDENCIA = 'E'";
	...
	public String Colaborador() {
		...
		db.con();
		String sql = "SELECT NOMBRE+' '+APATERNO+' '+AMATERNO AS COLABORADOR FROM COLABORADORES WHERE PK1 = "+this.getIdcolaborador()+"";
		String colaborador = "";
		ResultSet rs = db.getDatos(sql);
		try {
			if (rs != null) {
