...
public class mBoletosRobados  extends SuperModel {
	...
	public int contar(String search,int idsorteo) {
		...
		String sql = "SELECT B.PK1, I.INCIDENCIA";
		sql += " FROM SORTEOS_BOLETOS R, BOLETOS B,BOLETOS_INCIDENCIAS I WHERE R.PK_BOLETO = B.PK1 AND B.PK1 = I.PK_BOLETO AND "+condicion_Incidencias+" AND I.INCIDENCIA = 'R' AND R.PK_SORTEO = '"+idsorteo+"'";
	...
	String sql = "SELECT R.PK_BOLETO,B.VENDIDO,B.ABONO,B.PK1,B.FOLIO,B.COSTO, B.TALONARIO, B.FORMATO, B.SORTEO, B.PK_TALONARIO, B.INCIDENCIA, "
			...
			//+ "FROM SORTEOS_BOLETOS R, BOLETOS B, BOLETOS_INCIDENCIAS I WHERE R.PK_BOLETO = B.PK1 AND R.PK_BOLETO = I.PK_BOLETO AND R.PK_SORTEO = '"+idsorteo+"' AND I.INCIDENCIA = 'R'";
			+ "FROM SORTEOS_BOLETOS R, BOLETOS B, BOLETOS_INCIDENCIAS I WHERE R.PK_BOLETO = B.PK1 AND R.PK_BOLETO = I.PK_BOLETO AND R.PK_SORTEO = "+idsorteo+" AND "+condicion_Incidencias+" AND I.INCIDENCIA = 'R'";
	...
	//TOTAL BOLETOS  ROBADOS
	 sql = "SELECT COUNT(*) AS TOTAL FROM BOLETOS_INCIDENCIAS WHERE INCIDENCIA = 'R'  AND PK_SORTEO = '"+this.getIdSorteo()+"'";
		...
		//TALONARIOS NO VENDIDOS
		 sql = "SELECT COUNT(*) AS TOTAL FROM TALONARIOS WHERE SORTEO = '"+this.getIdSorteo()+"' AND VENDIDO = 'N'";
		 rs = db.getDatos(sql);
		 try {if (rs != null) { while(rs.next()){ total += rs.getString("TOTAL")+"|";}/*rs.close();*/}} catch (SQLException e) {e.printStackTrace();}
		 
		 
		//TOTAL BOLETOS NO VENDIDOS
		 sql = "SELECT COUNT(*) AS TOTAL FROM BOLETOS WHERE SORTEO = '"+this.getIdSorteo()+"' AND VENDIDO = 'N'";
		 rs = db.getDatos(sql);
		 try {if (rs != null) { while(rs.next()){ total += rs.getString("TOTAL")+"|";}/*rs.close();*/}} catch (SQLException e) {e.printStackTrace();}
    ...
    public void setComprador(){
     ...
     try {
		if (rs.next()) {
			 ...
			 if(this.getAbono() < this.getCosto()){ estatusventa = 'P'; }
			 if(this.getAbono() == this.getCosto()){ estatusventa = 'V'; }
			 if(this.getAbono() == 0){ estatusventa = 'N'; }
			 
			 sql = "UPDATE BOLETOS SET ABONO = "+this.getAbono()+", VENDIDO = '"+estatusventa+"'  WHERE SORTEO = "+this.getIdSorteo()+" AND TALONARIO = '"+this.getIdtalonario()+"' AND FOLIO="+ "'"+this.getBoleto()+"'";
			 db.execQuery(sql);
    ...
    public ResultSet BuscarBoletosTalonarios(){
   	 ...
   	 String sql = "SELECT R.PK_BOLETO,B.VENDIDO,B.ABONO,B.PK1,B.FOLIO,B.COSTO, B.TALONARIO, B.FORMATO, B.SORTEO, B.PK_TALONARIO, "
            + " (select TOP 1 VENDIDO FROM TALONARIOS WHERE PK1 = B.PK_TALONARIO) AS 'VENDIDOTALONARIO', "
			+ " (select TOP 1 PK_SECTOR  from SORTEOS_SECTORES_BOLETOS       where PK_BOLETO = R.PK_BOLETO) AS 'PK_SECTOR', "
			+ " (select TOP 1 SECTOR FROM SECTORES      WHERE PK1=(select TOP 1 PK_SECTOR      from SORTEOS_SECTORES_BOLETOS  where PK_BOLETO = R.PK_BOLETO)) AS 'SECTOR', "
			+ " (select TOP 1 PK_NICHO       from SORTEOS_NICHOS_BOLETOS where PK_BOLETO = R.PK_BOLETO) AS 'PK_NICHO', "
			...
			+ " (select TOP 1 PK_COLABORADOR from SORTEOS_COLABORADORES_BOLETOS  where PK_BOLETO = R.PK_BOLETO) AS 'PK_COLABORADOR', "
			+ " (select TOP 1 NOMBRE+' '+APATERNO  FROM COLABORADORES WHERE PK1=(select TOP 1 PK_COLABORADOR from SORTEOS_COLABORADORES_BOLETOS  where PK_BOLETO = R.PK_BOLETO)) AS 'COLABORADOR' "
			+ " FROM SORTEOS_BOLETOS R, BOLETOS B WHERE R.PK_BOLETO = B.PK1 AND  R.PK_SORTEO = "+this.getIdSorteo()+" AND TALONARIO = '"+this.getIdtalonario()+"' AND " + condicion_Incidencias;
