...
public class mBoletosRobadosNichos  extends SuperModel {
	...
	public int contar(String search,int idsorteo, int idsector, int idnicho) {
		...
		sql += " FROM SORTEOS_NICHOS_BOLETOS SB, BOLETOS B,BOLETOS_INCIDENCIAS I WHERE SB.PK_BOLETO = B.PK1 AND B.PK1 = I.PK_BOLETO AND I.INCIDENCIA = 'R' AND SB.PK_SORTEO = '"
				+ idsorteo
				+ "' AND SB.PK_SECTOR = '"
				+ idsector
				+ "' AND SB.PK_NICHO = '" + idnicho + "' ";
		if (search != "") {
	...
	public ResultSet paginacion(int pg, int numreg, String search,int idsorteo, int idsector, int idnicho) {
		...
		String sql = "SELECT S.PK_BOLETO,B.VENDIDO,B.ABONO,B.PK1,B.FOLIO,B.COSTO, B.TALONARIO, B.FORMATO, B.SORTEO, B.PK_TALONARIO, B.INCIDENCIA, "
			...
			+ "FROM SORTEOS_NICHOS_BOLETOS S, BOLETOS B, BOLETOS_INCIDENCIAS I WHERE S.PK_BOLETO = B.PK1 AND S.PK_BOLETO = I.PK_BOLETO AND S.PK_SORTEO = '"+idsorteo+"' AND S.PK_SECTOR = '"+idsector+"' AND S.PK_NICHO = '"+idnicho+"' AND I.INCIDENCIA = 'R'";
		...
		System.out.println(">>>>SQL ROB NICH:"+sql);
