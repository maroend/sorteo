...
public class mBoletosNOVendidosNichos extends SuperModel {
	...
	public mBoletosNOVendidosNichos() {
	...
	public int contar(String search,int idsorteo, int idsector, int idnicho) {
		
		String sql = "SELECT B.PK1";
		sql += " FROM SORTEOS_NICHOS_BOLETOS SB, BOLETOS B WHERE SB.PK_BOLETO = B.PK1 AND SB.PK_SORTEO = '"+idsorteo+"' AND B.VENDIDO ='N' AND SB.PK_SECTOR = '"+idsector+"' AND SB.PK_NICHO = '"+idnicho+"'";
	...
	String sql = "SELECT S.PK_BOLETO,B.VENDIDO,B.ABONO,B.PK1,B.FOLIO,B.COSTO, B.TALONARIO, B.FORMATO, B.SORTEO, B.PK_TALONARIO,B.INCIDENCIA, "
			...
			+ "FROM SORTEOS_NICHOS_BOLETOS S, BOLETOS B WHERE S.PK_BOLETO = B.PK1 AND S.PK_SORTEO = '"+idsorteo+"' AND S.PK_SECTOR = '"+idsector+"' AND S.PK_NICHO = '"+idnicho+"' AND B.VENDIDO='N'";
	 ...
	 public String getBoletosTalonariosNicho(){
		...
		//TOTAL BOLETOS  ROBADOS
		 sql = "SELECT COUNT(*) AS TOTAL FROM BOLETOS_INCIDENCIAS WHERE PK_SORTEO = "+this.getIdSorteo()+" AND PK_SECTOR = "+this.getIdsector()+" AND PK_NICHO = "+this.getIdnicho()+" AND INCIDENCIA = 'R'";
			 ...
			 sql = "SELECT COUNT(*) AS TOTAL FROM SORTEOS_NICHOS_TALONARIOS ST, TALONARIOS T " 
		                +" WHERE ST.PK_TALONARIO = T.PK1 AND T.VENDIDO = 'N' AND ST.PK_SORTEO = "+this.getIdSorteo()+" AND ST.PK_SECTOR = "+this.getIdsector()+" AND ST.PK_NICHO = "+this.getIdnicho()+"";
			 rs = db.getDatos(sql);
			 try {if (rs != null) { while(rs.next()){ total += rs.getString("TOTAL")+"|";}/*rs.close();*/}} catch (SQLException e) {e.printStackTrace();}
			...
			//TOTAL BOLETOS NO  VENDIDOS
			 sql = "SELECT COUNT(*) AS TOTAL FROM SORTEOS_NICHOS_BOLETOS SB, BOLETOS B "
		                +" WHERE SB.PK_BOLETO = B.PK1 AND B.VENDIDO = 'N' AND SB.PK_SORTEO = "+this.getIdSorteo()+" AND SB.PK_SECTOR = "+this.getIdsector()+" AND SB.PK_NICHO = "+this.getIdnicho()+"";
			 rs = db.getDatos(sql);
			 try {if (rs != null) { while(rs.next()){ total += rs.getString("TOTAL")+"|";}/*rs.close();*/}} catch (SQLException e) {e.printStackTrace();}	 
	 ...
	 public String getBoletosTalonariosSector()
	 {
		...
		//TALONARIOS NO VENDIDOS
		sql = "SELECT COUNT(*) AS TOTAL FROM SORTEOS_SECTORES_TALONARIOS ST, TALONARIOS T " 
			+" WHERE ST.PK_TALONARIO = T.PK1 AND T.VENDIDO = 'N' AND ST.PK_SORTEO = "+this.getIdSorteo()+" AND ST.PK_SECTOR = "+this.getIdsector()+" ";
			rs = db.getDatos(sql);
		try {if (rs != null) { while(rs.next()){ total += rs.getString("TOTAL")+"|";}/*rs.close();*/}} catch (SQLException e) {e.printStackTrace();}
		...
		//TOTAL BOLETOS NO VENDIDOS
		sql = "SELECT COUNT(*) AS TOTAL FROM SORTEOS_SECTORES_BOLETOS SB, BOLETOS B "
			+" WHERE SB.PK_BOLETO = B.PK1 AND B.VENDIDO = 'N' AND SB.PK_SORTEO = "+this.getIdSorteo()+" AND SB.PK_SECTOR = "+this.getIdsector()+" ";
			rs = db.getDatos(sql);
		try {if (rs != null) { while(rs.next()){ total += rs.getString("TOTAL")+"|";}/*rs.close();*/}} catch (SQLException e) {e.printStackTrace();}
