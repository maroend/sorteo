...
public class mBoletosVendidosXSector extends SuperModel  {
	...
	public mBoletosVendidosXSector() {
	...
	public int contar(String search,int idsorteo) {
		...
		sql += " FROM SORTEOS_SECTORES_BOLETOS SB, BOLETOS B WHERE SB.PK_BOLETO = B.PK1 AND SB.PK_SORTEO = '"+idsorteo+"'  AND SB.PK_SECTOR = "+this.getIdsector()+" AND B.VENDIDO ='V'";
		...
		System.out.println(">>>>SQL BOL COUNT: "+sql);
	...
	public ResultSet paginacion(int pg, int numreg, String search) {
		...
		String sql
			...
			+ " FROM SORTEOS_SECTORES_BOLETOS S, BOLETOS B WHERE S.PK_BOLETO = B.PK1 AND B.VENDIDO='V'"
			+ " AND S.PK_SORTEO = "+this.getIdSorteo()
			+ " AND S.PK_SECTOR = "+this.getIdsector();
    ...
    public String getBoletosTalonarios(){
 	...
 	//TALONARIOS ASIGNADOS
 	 sql = "SELECT COUNT(*) AS 'TOTAL' FROM SORTEOS_SECTORES_TALONARIOS R, TALONARIOS T WHERE PK_SORTEO="+this.getIdSorteo()+" AND PK_SECTOR="+this.getIdsector()+" AND ASIGNADO = 1";
 	 rs = db.getDatos(sql);
 	 try {if (rs != null) { while(rs.next()){ total += rs.getString("TOTAL")+"|";}/*rs.close();*/}} catch (SQLException e) {e.printStackTrace();}
 	 
 	 
 	//TALONARIOS PARCIALMENTE VENDIDOS
 	 sql = "SELECT COUNT(*) AS 'TOTAL' FROM SORTEOS_SECTORES_TALONARIOS R, TALONARIOS T WHERE R.PK_TALONARIO=T.PK1 AND R.PK_SORTEO="+this.getIdSorteo()+" AND R.PK_SECTOR="+this.getIdsector()+" AND T.VENDIDO='P'";
	 rs = db.getDatos(sql);
	 try {if (rs != null) { while(rs.next()){ total += rs.getString("TOTAL")+"|";}/*rs.close();*/}} catch (SQLException e) {e.printStackTrace();}
 	 
	//TALONARIOS VENDIDOS
	 sql = "SELECT COUNT(*) AS 'TOTAL' FROM SORTEOS_SECTORES_TALONARIOS R, TALONARIOS T WHERE R.PK_TALONARIO=T.PK1 AND R.PK_SORTEO="+this.getIdSorteo()+" AND R.PK_SECTOR="+this.getIdsector()+" AND T.VENDIDO='V'";
	 rs = db.getDatos(sql);
	 try {if (rs != null) { while(rs.next()){ total += rs.getString("TOTAL")+"|";}/*rs.close();*/}} catch (SQLException e) {e.printStackTrace();}
	...
	public String Sector() {
		try {
			...
			if (rs.next())
				this.setSector(rs.getString("SECTOR"));
		}catch(SQLException ex) { this.setSector(""); }

		return this.getSector();
	}
	
	public Object[][][] consultaEstadisticasXSector(SesionDatos sesion) {
		String nivel = "Sector";
		this.setIdSorteo((int)sesion.pkSorteo);
		this.setIdsector((int)sesion.pkSector);
		
		// ______ SECTORES _________
		ArrayList<String> poblaciones = new ArrayList<String>();
		poblaciones.add(this.Sector());

		ArrayList<Object> list_sec = new ArrayList<Object>();
		for (String str : poblaciones)
			list_sec.add(str);
		...
		list_1.add(new Integer(contarBoletosTotal(sesion, nivel)));
		list_2.add(new Integer(contarBoletosRetornados(sesion, nivel)));
		list_3.add(new Integer(contarBoletosVendidos(sesion, nivel)));
		list_4.add(new Integer(contarBoletosParcialmenteVendidos(sesion, nivel)));
		
		
		// _____ Objetos genericos para JSON ____
		
		ArrayList<Object> arrTmp = new ArrayList<Object>();
		
		arrTmp.add(new Long(sesion.pkSector));
