...
public class mSeguimientoColaboradores extends SuperModel {
	...
	public mSeguimientoColaboradores() {
		init();
	}
	...
	public int contar(int sector, int nicho, SesionDatos sesion) {
		...
		*/
				= "SELECT COUNT(*) AS 'MAX'"
				+ " FROM COLABORADORES C, NICHOS N, SECTORES S, SORTEOS_COLABORADORES_TALONARIOS SCT, TALONARIOS T"
				+ " WHERE S.PK_SORTEO=46 AND N.PK_SECTOR=S.PK1 AND C.PK_NICHO=N.PK1"
				+ " AND SCT.PK_TALONARIO=T.PK1 AND SCT.PK_COLABORADOR=C.PK1";
	...
	public ResultSet paginacion(int sector, int nicho, SesionDatos sesion) {
		...
		*/
		...
		String sql
			= " SELECT C.*,CONCAT(C.NOMBRE,' ',C.APATERNO) AS 'NOM_COLABORADOR', N.NICHO AS 'NOM_NICHO', S.SECTOR AS 'NOM_SECTOR', T.FOLIO, T.MONTO, T.VENDIDO, T.ABONO AS 'TALONARIO_ABONO'"
			+ " FROM COLABORADORES C, NICHOS N, SECTORES S, SORTEOS_COLABORADORES_TALONARIOS SCT, TALONARIOS T"
			+ " WHERE S.PK_SORTEO=" + sesion.pkSorteo
			+ " AND N.PK_SECTOR = S.PK1 AND C.PK_NICHO = N.PK1"
			+ " AND SCT.PK_TALONARIO=T.PK1 AND SCT.PK_COLABORADOR=C.PK1";
