...
public class mBoletosNOVendidosColaborador extends SuperModel {
	...
	public mBoletosNOVendidosColaborador() {
	...
	public int contar(String search, int idsorteo, int idsector, int idnicho,
		...
		sql += " FROM SORTEOS_COLABORADORES_BOLETOS SB, BOLETOS B WHERE SB.PK_BOLETO = B.PK1 AND SB.PK_SORTEO = '"
				+ idsorteo
				+ "' AND B.VENDIDO ='N' AND SB.PK_SECTOR = '"
				+ idsector
				+ "' AND SB.PK_NICHO = '"
				+ idnicho
				+ "' AND SB.PK_COLABORADOR = '" + idcolaborador + "'";
		...
		System.out.println(">>>>COLABORADOR NO VENDIDOS COUNT: " + sql);

		return numero;
	}

	public ResultSet paginacion(int pg, int numreg, String search,
			int idsorteo, int idsector, int idnicho, int idcolaborador) {
		...
		String sql = "SELECT S.PK_BOLETO,B.VENDIDO,B.ABONO,B.PK1,B.FOLIO,B.COSTO, B.TALONARIO, B.FORMATO, B.SORTEO, B.PK_TALONARIO,B.INCIDENCIA, "
				...
				+ "FROM SORTEOS_COLABORADORES_BOLETOS S, BOLETOS B WHERE S.PK_BOLETO = B.PK1 AND S.PK_SORTEO = '"
				+ idsorteo
				+ "' AND S.PK_SECTOR = '"
				+ idsector
				+ "' AND S.PK_NICHO = '"
				+ idnicho
				+ "' AND S.PK_COLABORADOR = '"
				+ idcolaborador
				+ "' AND B.VENDIDO='N'";
		...
		System.out.println("COLABORADOR NO VENDIDOS: " + sql);
		ResultSet rs = db.getDatos(sql);
	...
	public String Nicho() {
		...
		db.con();
		String nicho = "";
		String sql = "SELECT NICHO FROM NICHOS WHERE PK1 = "
				+ this.getIdnicho() + "";
