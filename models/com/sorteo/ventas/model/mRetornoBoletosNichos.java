...
public class mRetornoBoletosNichos  extends SuperModel {
	...
	public int contar(String search,int idsorteo,int idsector, int idnicho) {
		...
		sql += " FROM SORTEOS_NICHOS_BOLETOS SB, BOLETOS B WHERE SB.PK_BOLETO = B.PK1 AND SB.PK_SORTEO = '"+idsorteo+"' AND SB.PK_SECTOR = '"+idsector+"' AND SB.PK_NICHO = '"+idnicho+"' AND B.VENDIDO='V' AND B.RETORNADO = 1";
		int numero = 0;
		...
		System.out.println(">>> retorn nichos num sql: "+sql);
	...
	String sql = "SELECT S.PK_BOLETO,B.VENDIDO,B.ABONO,B.PK1,B.FOLIO,B.COSTO, B.TALONARIO, B.FORMATO, B.SORTEO, B.PK_TALONARIO, B.RETORNADO, B.INCIDENCIA,"
			...
			+ "FROM SORTEOS_NICHOS_BOLETOS S, BOLETOS B WHERE S.PK_BOLETO = B.PK1 AND S.PK_SORTEO = '"+idsorteo+"' AND S.PK_SECTOR = "+idsector+"  AND S.PK_NICHO = '"+idnicho+"' AND B.VENDIDO='V' AND B.RETORNADO = 1 ";
		...
		System.out.println(">>> retorn nichos sql: "+sql);
		ResultSet rs = db.getDatos(sql);
