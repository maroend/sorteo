...
public class mCompradoresColaborador extends SuperModel {
	...
	public mCompradoresColaborador() {
	...
	public int contar(String search,int idsorteo,int idsector,int idnicho,int idcolaborador) {
		...
		sql += " FROM SORTEOS_COLABORADORES_BOLETOS SB, BOLETOS B WHERE SB.PK_BOLETO = B.PK1 AND SB.PK_SORTEO = '"+idsorteo+"' AND SB.PK_SECTOR = '"+idsector+"'  AND SB.PK_NICHO = '"+idnicho+"' AND SB.PK_COLABORADOR = '"+idcolaborador+"' AND (B.VENDIDO='V' OR B.VENDIDO='P')";
		int numero = 0;
		...
		System.out.println(">>>sql comp num: "+sql);
	...
	String sql = "SELECT S.PK_BOLETO,B.VENDIDO,B.ABONO,B.PK1,B.FOLIO,B.COSTO, B.TALONARIO, B.FORMATO, B.SORTEO, B.PK_TALONARIO,B.INCIDENCIA, "
			...
			+"(select TOP 1 NOMBRE+' '+APELLIDOS  from COMPRADORES where PK_BOLETO = S.PK_BOLETO) AS 'COMPRADOR',"
			...
			+ "FROM SORTEOS_COLABORADORES_BOLETOS S, BOLETOS B WHERE S.PK_BOLETO = B.PK1 AND S.PK_SORTEO = '"+idsorteo+"' AND S.PK_SECTOR = "+idsector+"  AND S.PK_NICHO = '"+idnicho+"' AND S.PK_COLABORADOR = '"+idcolaborador+"' AND (B.VENDIDO='V' OR B.VENDIDO='P') ";
		...
		System.out.println(">>>sql comp: "+sql);
		ResultSet rs = db.getDatos(sql);
	...
	/*public int contar(int idsorteo) {
		String sql = "SELECT COUNT(*) AS TOTAL";
		sql += " FROM SORTEOS_BOLETOS SB, BOLETOS B WHERE SB.PK_BOLETO = B.PK1 AND SB.PK_SORTEO = '"+idsorteo+"' AND (B.VENDIDO='V' OR B.VENDIDO='P')";
        int numero = 0;
...
public void getUsuarioSorteo(){
	...
	String sql = "SELECT S.PK_BOLETO,B.VENDIDO,B.ABONO,B.PK1,B.FOLIO,B.COSTO, B.TALONARIO, B.FORMATO, B.SORTEO, B.PK_TALONARIO,B.INCIDENCIA, "
			...
			+ "(select TOP 1 PK_COLABORADOR from SORTEOS_COLABORADORES_BOLETOS  where PK_BOLETO = S.PK_BOLETO) AS 'PK_COLABORADOR',"
			+ " (select TOP 1 NOMBRE+' '+APATERNO  FROM COLABORADORES WHERE PK1=(select TOP 1 PK_COLABORADOR from SORTEOS_COLABORADORES_BOLETOS  where PK_BOLETO = S.PK_BOLETO)) AS 'COLABORADOR'"
			+ "FROM SORTEOS_BOLETOS S, BOLETOS B WHERE S.PK_BOLETO = B.PK1 AND S.PK_SORTEO = '"+idsorteo+"' AND (B.VENDIDO='V' OR B.VENDIDO='P')" ;
 ...
 public void Sorteo(){
	 ...
	 public String getBoletosTalonariosSector(){
	 	...
	 	System.out.println("sql getBoletosTalonarios Sector asignados: "+sql);
	 	 rs = db.getDatos(sql);
	 	 try {if (rs != null) { while(rs.next()){ total += rs.getString("TOTAL")+"|";}/*rs.close();*/}} catch (SQLException e) {e.printStackTrace();}
