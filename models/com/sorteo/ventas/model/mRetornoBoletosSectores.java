...
public class mRetornoBoletosSectores  extends SuperModel {
	...
	public mRetornoBoletosSectores() {
	...
	public int contar(String search,int idsorteo,int idsector) {
		...
		System.out.println(">>>sec retorn num sql: "+sql);
		ResultSet rs = db.getDatos(sql);
	   	 try {
				if (rs != null) {
	...
	String sql = "SELECT S.PK_BOLETO,B.VENDIDO,B.ABONO,B.PK1,B.FOLIO,B.COSTO, B.TALONARIO, B.FORMATO, B.SORTEO, B.PK_TALONARIO, B.RETORNADO, B.INCIDENCIA,"
		...
		System.out.println(">>>sec retorn sql: "+sql);
		ResultSet rs = db.getDatos(sql);
