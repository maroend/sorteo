...
public class mRetornoBoletosColaborador  extends SuperModel {
	...
	public mRetornoBoletosColaborador() {
	...
	public int contar( String search,int idsorteo,int idsector, int idnicho,int idcolaborador) {
		String sql = "SELECT COUNT(*) AS TOTAL ";
		sql += " FROM SORTEOS_COLABORADORES_BOLETOS SB, BOLETOS B WHERE SB.PK_BOLETO = B.PK1 AND SB.PK_SORTEO = '"+idsorteo+"' AND SB.PK_SECTOR = '"+idsector+"' AND SB.PK_NICHO = '"+idnicho+"' AND SB.PK_COLABORADOR = '"+idcolaborador+"' AND B.VENDIDO='V' AND B.RETORNADO = 1";
		int numero = 0;
		...
		System.out.println(">>> retorn colaborador num sql: "+sql);
		ResultSet rs = db.getDatos(sql);
	   	 try {
				if (rs != null) {
	...
	String sql = "SELECT S.PK_BOLETO,B.VENDIDO,B.ABONO,B.PK1,B.FOLIO,B.COSTO, B.TALONARIO, B.FORMATO, B.SORTEO, B.PK_TALONARIO, B.RETORNADO, B.INCIDENCIA,"
			...
			+ "FROM SORTEOS_COLABORADORES_BOLETOS S, BOLETOS B WHERE S.PK_BOLETO = B.PK1 AND S.PK_SORTEO = '"+idsorteo+"' AND S.PK_SECTOR = "+idsector+"  AND S.PK_NICHO = '"+idnicho+"' AND S.PK_COLABORADOR = '"+idcolaborador+"' AND B.VENDIDO='V' AND B.RETORNADO = 1 ";
		...
		System.out.println(">>> retorn colaborador sql: "+sql);
		ResultSet rs = db.getDatos(sql);
	 ...
	 public String getBoletosTalonariosColaborador(){
		...
		//TOTAL BOLETOS  ROBADOS
		 sql = "SELECT COUNT(*) AS TOTAL FROM BOLETOS_INCIDENCIAS WHERE PK_SORTEO = "+this.getIdSorteo()+" AND PK_SECTOR = "+this.getIdsector()+" AND PK_NICHO = "+this.getIdnicho()+" AND PK_COLABORADOR= "+this.getIcolaborador()+" AND INCIDENCIA = 'R'";
			...
			//TOTAL DE RETORNADOS
			 sql = "SELECT COUNT(*) AS TOTAL FROM SORTEOS_COLABORADORES_BOLETOS SB, BOLETOS B WHERE SB.PK_BOLETO = B.PK1 AND SB.PK_SORTEO = '" + this.getIdSorteo() + "' AND SB.PK_SECTOR = '"+this.getIdsector()+"' AND SB.PK_NICHO = "+this.getIdnicho()+"  AND SB.PK_COLABORADOR= "+this.getIcolaborador()+"  AND B.VENDIDO='V' AND B.RETORNADO = 1";
	 ...
	 public String getBoletosTalonariosNicho(){
		...
		//TOTAL DE RETORNADOS
		 sql = "SELECT COUNT(*) AS TOTAL FROM SORTEOS_NICHOS_BOLETOS SB, BOLETOS B WHERE SB.PK_BOLETO = B.PK1 AND SB.PK_SORTEO = '" + this.getIdSorteo() + "' AND SB.PK_SECTOR = '"+this.getIdsector()+"' AND SB.PK_NICHO = "+this.getIdnicho()+" AND B.VENDIDO='V' AND B.RETORNADO = 1";
		//System.out.println("sql getBoletosTalonarios: "+sql);
		 rs = db.getDatos(sql);
		try {if (rs != null) { while(rs.next()){ total += rs.getString("TOTAL")+"|";}/*rs.close();*/}} catch (SQLException e) {e.printStackTrace();}	 
	...
	//TOTAL DE RETORNADOS
	 sql = "SELECT COUNT(*) AS TOTAL FROM SORTEOS_SECTORES_BOLETOS SB, BOLETOS B WHERE SB.PK_BOLETO = B.PK1 AND SB.PK_SORTEO = '" + this.getIdSorteo() + "' AND SB.PK_SECTOR = '"+this.getIdsector()+"'  AND B.VENDIDO='V' AND B.RETORNADO = 1";
	//System.out.println("sql getBoletosTalonarios: "+sql);
	 rs = db.getDatos(sql);
	 try {if (rs != null) { while(rs.next()){ total += rs.getString("TOTAL")+"|";}/*rs.close();*/}} catch (SQLException e) {e.printStackTrace();}	 
    ...
    public String Nicho(){
	   	 ...
	   	 db.con();
	   	String nicho ="";
	   	 String sql = "SELECT NICHO FROM NICHOS WHERE PK1 = "+this.getIdnicho()+"";
    ...
    public String Colaborador(){
	   	 ...
	   	 db.con();
	   	 String sql = "SELECT NOMBRE+' '+APATERNO+' '+AMATERNO AS COLABORADOR FROM COLABORADORES WHERE PK1 = "+this.getIcolaborador()+"";
	 	String colaborador ="";
	   	 ResultSet rs = db.getDatos(sql);
	   	 try {
				if (rs != null) {
