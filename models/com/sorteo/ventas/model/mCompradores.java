...
public class mCompradores extends SuperModel {
	...
	public int contarSector(int idsorteo,int idsector) {
		...
		sql += " FROM SORTEOS_SECTORES_BOLETOS SB, BOLETOS B WHERE SB.PK_BOLETO = B.PK1 AND SB.PK_SORTEO = '"+idsorteo+"' AND SB.PK_SECTOR = '"+idsector+"' AND (B.VENDIDO='V' OR B.VENDIDO='P')";
		int numero = 0;
	...
	String sql = "SELECT S.PK_BOLETO,B.VENDIDO,B.ABONO,B.PK1,B.FOLIO,B.COSTO, B.TALONARIO, B.FORMATO, B.SORTEO, B.PK_TALONARIO,B.INCIDENCIA, "
			...
			+ "FROM SORTEOS_SECTORES_BOLETOS S, BOLETOS B WHERE S.PK_BOLETO = B.PK1 AND S.PK_SORTEO = '"+idsorteo+"' AND S.PK_SECTOR = "+idsector+"  AND (B.VENDIDO='V' OR B.VENDIDO='P') ";
