...
public class mCompradoresNichos extends SuperModel {
	...
	public int contar(String search,int idsorteo,int idsector,int idnicho) {
		...
		sql += " FROM SORTEOS_NICHOS_BOLETOS SB, BOLETOS B WHERE SB.PK_BOLETO = B.PK1 AND SB.PK_SORTEO = '"+idsorteo+"' AND SB.PK_SECTOR = '"+idsector+"'  AND SB.PK_NICHO = '"+idnicho+"' AND (B.VENDIDO='V' OR B.VENDIDO='P')";
		int numero = 0;
	...
	String sql = "SELECT S.PK_BOLETO,B.VENDIDO,B.ABONO,B.PK1,B.FOLIO,B.COSTO, B.TALONARIO, B.FORMATO, B.SORTEO, B.PK_TALONARIO,B.INCIDENCIA, "
			...
			+ "FROM SORTEOS_NICHOS_BOLETOS S, BOLETOS B WHERE S.PK_BOLETO = B.PK1 AND S.PK_SORTEO = '"+idsorteo+"' AND S.PK_SECTOR = "+idsector+"  AND S.PK_NICHO = '"+idnicho+"' AND (B.VENDIDO='V' OR B.VENDIDO='P') ";
