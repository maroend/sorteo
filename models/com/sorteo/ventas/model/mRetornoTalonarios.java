...
public class mRetornoTalonarios  extends SuperModel {
	...
	public void totalregistros(int totalregistros) {
	...
	public int contar(int idsorteo, String search,int pg, int numreg) {
		//String sql = "SELECT B.PK1";
		//sql += " FROM SORTEOS_BOLETOS SB, BOLETOS B WHERE SB.PK_BOLETO = B.PK1 AND SB.PK_SORTEO = '"+idsorteo+"' AND B.VENDIDO ='V' AND B.RETORNADO = 1";
		
		String	buscar = "";
		if (search != "") {
			buscar = " AND (T.FOLIO = '" + search + "')";	}				
	
		
		String sql = "SELECT  COUNT(PK1) AS 'TALONARIOSRETORNADOS' "
				+ "FROM  ( SELECT  T.PK1,"
				+ "  ( SELECT  COUNT(B.PK1) AS 'BRETORNADOS' FROM BOLETOS B WHERE B.TALONARIO = S.PK_TALONARIO AND B.SORTEO = '"+idsorteo+"'  AND B.VENDIDO = 'V' AND B.RETORNADO = 1 ) AS 'BRETORNADOS' "
				+ "FROM SORTEOS_TALONARIOS S, TALONARIOS T WHERE S.PK_TALONARIO = T.PK1 AND S.PK_SORTEO = '"+idsorteo+"' AND T.VENDIDO='V' "+ buscar + " ORDER BY CAST(T.PK1 AS INT) ASC OFFSET (" + (pg - 1) * numreg + ") ROWS FETCH NEXT " + numreg + " ROWS ONLY) "
	...
	try {
		...
		if (rs != null) 			
			 if(rs.next())
				 numero = rs.getInt("TALONARIOSRETORNADOS"); 		
	...
	public ResultSet paginacion(int pg, int numreg, String search,int idsorteo) {
		...
		String sql = "SELECT  *"
				+ " FROM  ( SELECT  S.PK_TALONARIO,T.VENDIDO,T.ABONO,T.PK1,T.MONTO, T.FOLIO, T.FORMATO, T.SORTEO, "
				+ " ( SELECT  COUNT(B.PK1) AS 'BRETORNADOS' FROM BOLETOS B WHERE B.TALONARIO = S.PK_TALONARIO AND B.SORTEO = '"+idsorteo+"'  AND B.VENDIDO = 'V' AND B.RETORNADO = 1 ) AS 'BRETORNADOS',"
	...
	public int contarSector(int idsorteo,int idsector) {
		String sql = "SELECT COUNT(*) AS TOTAL ";
		sql += " FROM SORTEOS_SECTORES_BOLETOS SB, BOLETOS B WHERE SB.PK_BOLETO = B.PK1 AND SB.PK_SORTEO = '"+idsorteo+"' AND SB.PK_SECTOR = '"+idsector+"' AND B.VENDIDO='V' AND B.RETORNADO = 1";
		int numero = 0;
	...
	public ResultSet paginacionSector(int pg, int numreg, String search,int idsorteo,int idsector) {
	...
	String sql = "SELECT S.PK_BOLETO,B.VENDIDO,B.ABONO,B.PK1,B.FOLIO,B.COSTO, B.TALONARIO, B.FORMATO, B.SORTEO, B.PK_TALONARIO, B.RETORNADO, B.INCIDENCIA,"
			...
			+ "FROM SORTEOS_SECTORES_BOLETOS S, BOLETOS B WHERE S.PK_BOLETO = B.PK1 AND S.PK_SORTEO = '"+idsorteo+"' AND S.PK_SECTOR = "+idsector+"  AND B.VENDIDO='V' AND B.RETORNADO = 1 ";
	...
	//TOTAL BOLETOS  ROBADOS
	 sql = "SELECT COUNT(*) AS TOTAL FROM BOLETOS_INCIDENCIAS WHERE INCIDENCIA = 'R'  AND PK_SORTEO = '"+this.getIdSorteo()+"'";
		...
		//TOTAL DE RETORNADOS
		 sql = "SELECT COUNT(*) AS TOTAL FROM BOLETOS WHERE SORTEO = '"+this.getIdSorteo()+"'  AND VENDIDO='V' AND RETORNADO = 1";
		 rs = db.getDatos(sql);
		 try {if (rs != null) { while(rs.next()){ total += rs.getString("TOTAL")+"|";}/*rs.close();*/}} catch (SQLException e) {e.printStackTrace();}	 
