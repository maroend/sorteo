...
import com.sorteo.talonarios.model.mSeguimientoBoletos;

/**
 * Servlet implementation class BoletosSorteo
 */
@WebServlet("/SeguimientoBoletos")
public class SeguimientoBoletos extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request,
		...
		SesionDatos sesion;
		mSeguimientoBoletos model = new mSeguimientoBoletos();
		
		if ((sesion = SesionDatos.start(request, response, false, model)) == null)
			return;
		
		System.out.println("************* SeguimientoBoletos *************");
		...
		default:
			...
			if ((str_idtalonario = request.getParameter("idboleto")) != null) {
				int idboleto = Integer.valueOf(str_idtalonario);
				if (sesion.pkBoleto != idboleto) {
					sesion.pkBoleto = idboleto;
					guardar = true;
				}
			}
			if(guardar)
			...
			fullPath = "/WEB-INF/views/boveda/SeguimientoBoletos/seguimiento_boletos.html";
			...
			model.setIdBoleto(sesion.pkBoleto);
			String folio = model.folioBoleto();
			HTML = HTML.replaceFirst("#FOLIO#", folio);

			//HTML = this.Contenido(HTML,idSorteo);
	...
	protected void Buscar(HttpServletRequest request,
			...
			mSeguimientoBoletos model, SesionDatos sesion) throws ServletException, IOException {

		String[] columnas = { "Estado", "Costo", "Abono", "Formato", "Fecha", "Acci&oacute;n", "Detalle" };
		String[] campos = { "ESTATUS", "COSTO", "ABONO", "FORMATO", "FECHA_R", "ACCION", "DETALLE" };
	...
	protected String CrearTabla(int numreg, ResultSet rs, String[] columnas, String[] campos, int pg, int show, mSeguimientoBoletos model) {
			...
			while (rs.next()) {
				...
				for (String campo : campos) {
					if (campo.equals("DETALLE")) {
						...
						else if (nivel.equals("Colaborador") || nivel.equals("Comprador")) nivel_nombre = "Colaborador: " + model.Colaborador(rs.getInt("PK_COLABORADOR"));
						else nivel_nombre = "";
