...
import com.sorteo.conciliacion.model.mConciliacion;
 ...
 */
@WebServlet("/Conciliacion")
public class Conciliacion extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request,
		...
		mConciliacion model = new mConciliacion();
	...
	//	fullPath = "/WEB-INF/views/listsorteosuser.html";
		...
		case "Buscar":
			...
			HTML = Buscar(request, response, pg, show, search, sector, nicho, model, sesion);
			break;
		...
		default:
			...
			//fullPath = "/WEB-INF/views/conciliacion/Conciliacion_____.html";
			fullPath = "/WEB-INF/views/conciliacion/Conciliacion.html";
	...
	protected void actualizarAbono(HttpServletRequest request,
			HttpServletResponse response, mConciliacion model)
	...
	protected void BuscarSector(HttpServletRequest request,
			HttpServletResponse response, mConciliacion model, SesionDatos sesion) throws ServletException, IOException {
	...
	protected void buscarNichosSector(HttpServletRequest request,
			HttpServletResponse response, mConciliacion model) throws ServletException, IOException {
	...
	protected String Buscar(HttpServletRequest request,
			...
			int sector, int nicho, mConciliacion model, SesionDatos sesion) throws ServletException, IOException {

		String[] columnas = { "Clave", "Nombre", "Monto","Comisi&oacute;n","Abono","Saldo"};
		String[] campos   = { "CLAVE", "NOMBRE", "MONTO","COMISION","ABONO","SALDO" };
		...
		return CrearTabla(numeroregistros,
				model.paginacion(pg, show, search, OFFSET.TRUE),
			...
			if (Integer.valueOf(numreg) > 0)
			{
				...
				while (rs.next()) {
					...
					for (String campo : campos)
					{
						...
						if (numeros) sb.append("<b>");
						if (value==null) { sb.append("N/A"); }
						else {
							if (numeros) sb.append("$"); 
							sb.append(value);
						}
						if (numeros) sb.append("</b>");
					...
					.append("<b>$").append(rs.getString("ABONO")).append("</b>")
					.append("</td>");
	...
	protected void export(HttpServletRequest request,
			HttpServletResponse response, int pg, int show, String search,
			int sector, int nicho, mConciliacion model, SesionDatos sesion) throws ServletException, IOException, SQLException {
