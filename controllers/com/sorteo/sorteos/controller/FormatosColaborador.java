...
import com.sorteo.sorteos.model.mFormatosColaborador;

/**
 * Servlet implementation class FormatosColaborador
 */
@WebServlet("/FormatosColaborador")
public class FormatosColaborador extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		SesionDatos sesion;
		mFormatosColaborador model = new mFormatosColaborador();
		...
		if (!sesion.permisos.havePermiso(10177)) { view = "error"; }
		...
		default:
			...
			fullPath = "/WEB-INF/views/sorteos/formatos/FormatosColaborador.html";
			menu = vista.initMenu(0, false, 2, sesion.misSorteos==0 ? 34 : 34, sesion);
			notificaciones = vista.initNotificaciones(context,
					sesion.nombreCompleto);
			infouser = vista.initInfoUser(context, sesion.nombreCompleto,sesion.role);
			...
			model.obtenerColaborador(model);	
	...
	protected void Buscar(HttpServletRequest request,
			HttpServletResponse response, int pg, int show, String search, mFormatosColaborador model, SesionDatos sesion)
			throws ServletException, IOException {

		String[] columnas = { "Folio","Id boleto","Id Sector","id Nicho","id Colaborador","Estado","Usuario","Fecha Registro"};
		String[] campos = { "FOLIO","PK_BOLETO","PK_SECTOR","PK_NICHO","PK_COLABORADOR","PK_ESTADO","USUARIO","FECHA_R" };

		int numeroregistros = model.contar(sesion.pkSorteo,sesion.pkSector,sesion.pkNicho,sesion.pkColaborador);

		
		String HTML = CrearTabla(numeroregistros, model.paginacion(pg, show, search,sesion.pkSorteo,sesion.pkSector,sesion.pkNicho,sesion.pkColaborador), columnas, campos, pg, show);
		PrintWriter out = response.getWriter();
		out.println(HTML);
