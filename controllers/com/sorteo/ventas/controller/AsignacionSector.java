...
import com.sorteo.ventas.model.mAsignacionSector;

/**
 * Servlet implementation class AsignacionSector
 */
@WebServlet("/AsignacionSector")
public class AsignacionSector extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		SesionDatos sesion;
		mAsignacionSector model = new mAsignacionSector();
		...
		default:
			fullPath = "/WEB-INF/views/ventas/seguimientosector.html";
			menu = vista.initMenu(0, false, 19, 11, sesion);
			notificaciones = vista.initNotificaciones(context,
					sesion.nombreCompleto);
			infouser = vista.initInfoUser(context, sesion.nombreCompleto,sesion.role);
	...
	protected void SorteoPredeterminado(HttpServletRequest request,
			HttpServletResponse response, String HTML, mAsignacionSector model, SesionDatos sesion) throws ServletException,
			IOException {
	...
	private String Contenido(String HTML,int idsorteo, int idsector, mAsignacionSector model){
	...
	//	int numboletosasignados = 0;
		...
		try {
			while (rs.next()) {
				  ...
				  //boletosentregados += Integer.parseInt(model.TotalboletosSector(model));
				  
				  boletosxentregar = Integer.parseInt(model.TotalboletosSector(model)) - Integer.parseInt(model.TotalboletosSectorAsignados(model));
				  
				  totalboletosretornados += model.BoletosRetornados();
				  
				  totalboletosasignados += Integer.parseInt(model.TotalboletosSectorAsignados(model));
				  
				  dato += "<tr>";
				  dato += "<td><span style=\"display:none;\" id=\"sector"+rs.getString("PK1")+"\" onClick=\"showNichos("+rs.getString("PK1")+")\"><i class=\"fa fa-lg fa-plus-square\"></i></span><a href=\"AsignacionNicho?sorteo="+idsorteo+"&sector="+rs.getString("PK1")+"\">"+rs.getString("SECTOR")+"</a></td>";
				  dato += " <td><span class=\"price-money\">"+model.TotalboletosSector(model)+"</span></td>";
				  dato += " <td><span class=\"price-money\">"+model.TotalboletosSectorAsignados(model)+"</span></td>";  
