...
import com.sorteo.ventas.model.mRetornoTalonarios;

/**
 * Servlet implementation class RetornoTalonarios
 */
@WebServlet("/RetornoTalonarios")
public class RetornoTalonarios extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		SesionDatos sesion;
		mRetornoTalonarios model = new mRetornoTalonarios();
	   ...
	   case "GetComprador":
		...
		default:
			...
			fullPath = "/WEB-INF/views/ventas/retornotalonarios.html";
			menu = vista.initMenu(0, false, 19, 13, sesion);
			notificaciones = vista.initNotificaciones(context,
					sesion.nombreCompleto);
			infouser = vista.initInfoUser(context, sesion.nombreCompleto,sesion.role);
			...
			dato = "RetornoBoletos?idsorteo="+sesion.pkSorteo;
			regex = "#LINKBOLETOS#";
			HTML = HTML.replaceAll(regex, dato);  
			
			 dato = "RetornoTalonarios?idsorteo="+sesion.pkSorteo;
			regex = "#LINKTALONARIOS#";
			HTML = HTML.replaceAll(regex, dato);  
	...
	protected String getSorteosUsuarios(HttpServletRequest request, HttpServletResponse response, String HTML, SesionDatos sesion) throws ServletException, IOException {
	 ...
	 protected void SorteoPredeterminado(HttpServletRequest request, HttpServletResponse response, String HTML, mRetornoTalonarios model, SesionDatos sesion) throws ServletException, IOException {
		   ...
		   System.out.println(sorteo);
					...
					if(sorteo!=0 && sector==0 && nicho==0){
						...
						response.sendRedirect("RetornoBoletos?view=listsorteos");
					}
	   }
	
	
	
	 protected String getBoletosTalonarios(mRetornoTalonarios model, SesionDatos sesion){
   ...
   protected String getBoletosTalonariosSector(int idsorteo, int idsector, mRetornoTalonarios model){
  ...
  protected String getBoletosTalonariosNicho(int idsorteo, int idsector,int idnicho, mRetornoTalonarios model){
  ...
  protected String getBoletosTalonariosColaborador(int idsorteo, int idsector,int idnicho, int idcolaborador, mRetornoTalonarios model){
	  ...
	  String contenido = "";
	    ...
	    protected String GetComprador(int idsorteo, String boleto, String talonario, mRetornoTalonarios model){
   ...
   protected String GetIncidenciaBoleto(int idsorteo, String boleto, String talonario, mRetornoTalonarios model){
   ...
   protected String BuscarMontoAbonoTalonario(int idsorteo, String talonario, mRetornoTalonarios model){
   ...
   protected String BuscarBoletosTalonarios(int idsorteo, String talonario, mRetornoTalonarios model){
	   String contenido = "";
	   ResultSet rs = null;
	   String estado = "";
	...
	protected void Buscar(HttpServletRequest request,
			HttpServletResponse response, int pg, int show, String search, mRetornoTalonarios model, SesionDatos sesion)
			throws ServletException, IOException {

		/*String[] columnas = { "Talonario","Estatus","Costo","Abono","Sector","Nicho","Colaborador"};
		String[] campos = { "FOLIO","VENDIDO","COSTO","ABONO","SECTOR","NICHO","COLABORADOR" };*/
		
		String[] columnas = { "Talonario","Monto","Abono","Boletos Retornados","Sector","Nicho","Colaborador"};
		String[] campos = { "FOLIO","MONTO","ABONO","BRETORNADOS","SECTOR","NICHO","COLABORADOR" };

        int numeroregistros = model.contar(sesion.pkSorteo,search,pg, show);
		...
		if (Integer.valueOf(numreg) > 0) {		
			...
			while (rs.next()) {
               ...
               // html += "<td class=\"sorting_1\"><a href=\"javascript:ShowDetalleBoleto('"+sesion.pkSorteo+"','"+rs.getString("PK_BOLETO")+"','"+rs.getString("FOLIO")+"','"+rs.getString("PK_TALONARIO")+"','"+rs.getString("TALONARIO")+"','"+rs.getString("ABONO")+"','"+rs.getString("COSTO")+"','"+rs.getString("PK_SECTOR")+"','"+rs.getString("PK_NICHO")+"','"+rs.getString("PK_COLABORADOR")+"')\">";
				//html += "<td class=\"sorting_1\"><a href=\"\">";
				...
				// html += "<span class=\"label label-primary\" >"+rs.getString("FOLIO")+"</span>";
				

				if(rs.getString("VENDIDO").equals("N")){  estadotalonario = "<span class=\"label label-inverse\" >"+rs.getString("PK1")+"</span>";}
				if(rs.getString("VENDIDO").equals("V")){  estadotalonario = "<span class=\"label label-success\" >"+rs.getString("PK1")+"</span>";}
				if(rs.getString("VENDIDO").equals("P")){  estadotalonario = "<span class=\"label label-default\" >"+rs.getString("PK1")+"</span>";}
				
				
				html += "<td class=\"sorting_1\"><a href=\"javascript:ShowDetalleTalonario('"+sesion.pkSorteo+"','"+rs.getString("PK_TALONARIO")+"','"+rs.getString("PK_TALONARIO")+"')\">" + estadotalonario
						+ "</a></td>";
				...
				html += "<td class=\"sorting_1\">" + rs.getString("MONTO")
						+ "</td>";
				...
				html += "<td class=\"sorting_1\">" + rs.getString("BRETORNADOS")
						+ "</td>";
				...
				html += "<td class=\"sorting_1\">" + ((rs.getString("SECTOR") == null) ?  "N/A" : "<a href=\"RetornoBoletosSectores?idsorteo="+sesion.pkSorteo+"&idsector="+ rs.getString("PK_SECTOR")+  "\">"+StringEscapeUtils.escapeHtml4(rs.getString("SECTOR"))+"</a>")
						+ "</td>";
				...
				html += "<h1>No existen Talonarios Retornados</h1>";
