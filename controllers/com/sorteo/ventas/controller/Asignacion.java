...
import com.sorteo.ventas.model.mAsignacion;
...
public class Asignacion extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request,
		...
		mAsignacion model = new mAsignacion();
		SesionDatos sesion;
		...
		default:
			fullPath = "/WEB-INF/views/ventas/seguimientosorteo.html";
			menu = vista.initMenu(0, false, 19, 11, sesion);
			notificaciones = vista.initNotificaciones(context,
					sesion.nombreCompleto);
			infouser = vista.initInfoUser(context, sesion.nombreCompleto,sesion.role);
	...
	protected void SorteoPredeterminado(HttpServletRequest request,
			HttpServletResponse response, String HTML, mAsignacion model,
		...
		long nicho = model.getIdnicho();
					...
					if(sorteo!=0 && sector==0 && nicho==0){
						...
						response.sendRedirect("Asignacion?view=listsorteos");
					}
	}

	private String Contenido(String HTML, int idsorteo, mAsignacion model) {
		...
		try {
			while (rs.next()) {
				...
				boletosentregados = model.TotalboletosSector(model);
				boletosentregadosSector = model.TotalboletosSectorAsignados(model);
				
				
				boletosxentregar = boletosentregados - boletosentregadosSector;
				
				totalboletosretornados += model.BoletosRetornados();
				
				totalboletosasignados += boletosentregadosSector;
				
				dato += "<tr>";
				dato += "<td><span style=\"display:none;\" id=\"sector"+rs.getString("PK1")+"\" onClick=\"showNichos("+rs.getString("PK1")+")\"><i class=\"fa fa-lg fa-plus-square\"></i></span><a href=\"AsignacionSector?sorteo="+idsorteo+"&sector="+rs.getString("PK1")+"\">"+rs.getString("SECTOR")+"</a></td>";
				dato += " <td><span class=\"price-money\">"+boletosentregados+"</span></td>";
				dato += " <td><span class=\"price-money\">"+boletosentregadosSector+"</span></td>";  
