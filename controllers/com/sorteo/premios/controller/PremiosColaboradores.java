...
import com.sorteo.premios.model.mPremiosColaboradores;

/**
 * Servlet implementation class Premios
 */
@WebServlet("/PremiosColaboradores")
public class PremiosColaboradores extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		SesionDatos sesion;
		mPremiosColaboradores model = new mPremiosColaboradores();
		...
		case "Agregar":
			fullPath = "/WEB-INF/views/premios/agregar_premio_colaboradores.html";
			...
			HTML = HTML.replaceFirst("#NUM_PREMIO#", ""+next);
			break;
		...
		case "Buscar":
			...
			//System.out.println("  ---> " + sesion.pkSorteo);
		...
		default:
			...
			fullPath = "/WEB-INF/views/premios/lista_premios_colaboradores.html";
			menu = vista.initMenu(0, false, 16, 17, sesion);
			notificaciones = vista.initNotificaciones(context, sesion.nombreCompleto);
			infouser = vista.initInfoUser(context, sesion.nombreCompleto,sesion.role);
			HTML = vista.CrearVista(context, fullPath, menu, notificaciones, infouser);
			
			// XXXXXXXXXXXXXXXXXXXXXXXXXXX FALTA PERMISO  XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
			HTML = putCode(HTML, 20102,"#BTN_AGREGAR_PREMIO#", "",
					"<a class=\"btn btn-primary m-r-5\" href=\"PremiosColaboradores?view=Agregar\">Agregar premio</a>",
					sesion);
	...
	private void borraPremio(HttpServletRequest request,
			HttpServletResponse response, mPremiosColaboradores model, SesionDatos sesion) throws ServletException, IOException {
		// TODO Auto-generated method stub
		...
		response.sendRedirect("PremiosColaboradores?idsorteo=" + model.getIdsorteo());
	}
	
	private String Buscar(HttpServletRequest request,
			HttpServletResponse response, int pg, int show, String search,
			mPremiosColaboradores model, SesionDatos sesion) {
		model.setIdsorteo(sesion.pkSorteo);

		int numeroregistros = model.contarPremios();

		return CrearTabla(numeroregistros, model.paginacion(pg, show, search), pg, show, sesion);
	}
	...
	protected String CrearTabla(int numreg, ResultSet rs, int pg, int show, SesionDatos sesion) {
		...
		try {
			if (rs != null)
				while (rs.next()) {
					...
					if (sesion.permisos.havePermiso(20103)) { // permiso para borrar premios
						sb
						.append("  <div class=\"btn-row\"><a data-title=\"Eliminar premio\" data-container=\"body\" data-toggle=\"tooltip\" href=\"Premios?view=Borrar&idPremio=")
						.append(rs.getString("PK1"))
						.append("\" data-original-title=\"\" title=\"\"><i class=\"fa fa-fw fa-times-circle\"></i><span style=\"font-size:11px;\">Borra premio</span></a></div>\n");
					}
					sb.append(" </div>\n</div>\n\n");
					*/
				}
		...
		if (count == 0) {
			...
			sb.append("    <h1>No existen Ganadores</h1>");
			sb.append("    <p>Empieze por agregar un nuevo ganador.</p>");
			sb.append("  </div>");
			sb.append("</li>");
			sb.append("</ul>");
		}
	...
	protected void SorteoPredeterminado(HttpServletRequest request,
			HttpServletResponse response, mPremiosColaboradores model, SesionDatos sesion)
