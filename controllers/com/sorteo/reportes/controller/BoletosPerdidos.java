...
import com.sorteo.reportes.model.mBoletosPerdidos;

/**
 * Servlet implementation class BoletosPerdidos
 */
@WebServlet("/BoletosExtraviados")
public class BoletosPerdidos extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		SesionDatos sesion;
		mBoletosPerdidos model = new mBoletosPerdidos();
		...
		case "listsorteos":
			...
			menu = vista.initMenu(0, false, 19, 22, sesion);
	   ...
	   case "GetComprador":
		...
		default:
			...
			fullPath = "/WEB-INF/views/reportes/boletosperdidos.html";
			menu = vista.initMenu(0, false, 19, 22, sesion);
			notificaciones = vista.initNotificaciones(context,
					sesion.nombreCompleto);
			infouser = vista.initInfoUser(context, sesion.nombreCompleto,sesion.role);
	...
	protected String getSorteosUsuarios(HttpServletRequest request,
		...
		int idsorteo = modelDashboard.getIdSorteo();
		int idsector = modelDashboard.getIdSector();
		int idnicho = modelDashboard.getIdNicho();
	...
	protected void SorteoPredeterminado(HttpServletRequest request,
			HttpServletResponse response, String HTML, mBoletosPerdidos model, SesionDatos sesion)
		...
		if (idsorteo != 0 && idsector == 0 && idnicho == 0) {
			...
			response.sendRedirect("BoletosExtraviados?view=listsorteos");
		}
	}
	
	protected String getBoletosTalonarios(mBoletosPerdidos model,
			SesionDatos sesion) {
	...
	protected String getBoletosTalonariosNicho(int idsorteo, int idsector,
			int idnicho, mBoletosPerdidos model) {
	...
	protected String getBoletosTalonariosColaborador(int idsorteo,
			int idsector, int idnicho, int idcolaborador, mBoletosPerdidos model) {
	...
	protected String GetComprador(int idsorteo, String boleto,
			String talonario, mBoletosPerdidos model) {
	...
	protected void Buscar(HttpServletRequest request,
			...
			mBoletosPerdidos model, SesionDatos sesion)
			throws ServletException, IOException {
	...
	protected String CrearTabla(int numreg, ResultSet rs, String[] columnas,
		...
		if (Integer.valueOf(numreg) > 0) {		
			...
			html += "<h1>No existen Boletos Perdidos </h1>";		
			html += "</div>";
			html += "</td></tr>";
	   }			
