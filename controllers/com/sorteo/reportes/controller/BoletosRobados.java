...
import com.sorteo.reportes.model.mBoletosRobados;

/**
 * Servlet implementation class BoletosRobados
 */
@WebServlet("/BoletosRobados")
public class BoletosRobados extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		SesionDatos sesion;
		mBoletosRobados model = new mBoletosRobados();
		...
		if (view == null) {
			...
			if (request.getParameter("idsorteo") != null) {
				System.out.println("entra sorteo en default");
				// fullPath = "";
				view = "";
		...
		case "listsorteos":
			menu = vista.initMenu(0, false, 19, 1033, sesion);
		...
		default:
			...
			fullPath = "/WEB-INF/views/reportes/boletosrobados.html";
	...
	protected void SorteoPredeterminado(HttpServletRequest request,
			HttpServletResponse response, String HTML, mBoletosRobados model,
		...
		if (idsorteo != 0 && idsector == 0 && idnicho == 0) {
			...
			response.sendRedirect("BoletosRobadosSectores?idsorteo=" + idsorteo
					+ "&idsector=" + idsector);
		} else if (idsorteo != 0 && idsector != 0 && idnicho != 0) {
			response.sendRedirect("BoletosRobadosNichos?idsorteo=" + idsorteo
					+ "&idsector=" + idsector + "&idnicho=" + idnicho);
		} else {
			response.sendRedirect("BoletosRobados?view=listsorteos");
		}
	}

	protected String getBoletosTalonarios(mBoletosRobados model,
			SesionDatos sesion) {
	...
	protected String getBoletosTalonariosNicho(int idsorteo, int idsector,
			int idnicho, mBoletosRobados model) {
	...
	protected String GetComprador(int idsorteo, String boleto,
			String talonario, mBoletosRobados model) {
	...
	protected void Buscar(HttpServletRequest request,
			HttpServletResponse response, int pg, int show, String search, mBoletosRobados model, SesionDatos sesion)
	...
	if (Integer.valueOf(numreg) > 0) {			
		...
		html += "<h1>No existen Boletos Robados </h1>";		
		html += "</div>";
		html += "</td></tr>";
   }				
