...
import com.sorteo.reportes.model.mBoletosPerdidosNichos;

/**
 * Servlet implementation class BoletosPerdidosNichos
 */
@WebServlet("/BoletosPerdidosNichos")
public class BoletosPerdidosNichos extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		SesionDatos sesion;
		mBoletosPerdidosNichos model = new mBoletosPerdidosNichos();
		...
		if (!sesion.permisos.havePermiso(20097)){view = "error";}
		...
		case "error":
			...
			menu = vista.initMenu(0, false, 19, sesion.misSorteos == 0 ? 22
					: 19, sesion);
		...
		default:
			...
			fullPath = "/WEB-INF/views/reportes/boletosperdidosnichos.html";
			menu = vista.initMenu(0, false, 19, sesion.misSorteos == 0 ? 22 : 19, sesion);
	...
	protected void SorteoPredeterminado(HttpServletRequest request,
			...
			mBoletosPerdidosNichos model, SesionDatos sesion)
		...
		if (idsorteo != 0 && idsector == 0 && idnicho == 0) {
			response.sendRedirect("BoletosExtraviados?idsorteo=" + idsorteo);
		} else if (idsorteo != 0 && idsector != 0 && idnicho == 0) {
			response.sendRedirect("BoletosPerdidosSectores?idsorteo=" + idsorteo + "&idsector=" + idsector);
		} else if (idsorteo != 0 && idsector != 0 && idnicho != 0) {
			response.sendRedirect("BoletosPerdidosNichos?idsorteo=" + idsorteo + "&idsector=" + idsector + "&idnicho=" + idnicho);
		}
	}

	protected String getBoletosTalonarios(mBoletosPerdidosNichos model, SesionDatos sesion) {
	...
	protected String getBoletosTalonariosSector(int idsorteo, int idsector, mBoletosPerdidosNichos model) {
	...
	protected String getBoletosTalonariosNicho(int idsorteo, int idsector,
			int idnicho, mBoletosPerdidosNichos model) {
	...
	protected String getBoletosTalonariosColaborador(int idsorteo,
			int idsector, int idnicho, int idcolaborador, mBoletosPerdidosNichos model) {
	...
	protected String GetComprador(int idsorteo, String boleto, String talonario, mBoletosPerdidosNichos model){
   ...
   protected String GetIncidenciaBoleto(int idsorteo, String boleto, String talonario, mBoletosPerdidosNichos model){
   ...
   protected String BuscarBoletosTalonarios(int idsorteo, String talonario, mBoletosPerdidosNichos model){
	   String contenido = "";
	   ResultSet rs = null;
	   String estado = "";
	...
	if (Integer.valueOf(numreg) > 0) {		
			...
			while (rs.next()) {
				...
				/*html += "<td class=\"sorting_1\">" + ((rs.getString("SECTOR") == null) ?  "N/A" : "<a href=\"BoletosPerdidosSectores?idsorteo="+sesion.pkSorteo+"&idsector="+ rs.getString("PK_SECTOR")+  "\">"+StringEscapeUtils.escapeHtml4(rs.getString("SECTOR"))+"</a>")
						+ "</td>";
				
				html += "<td class=\"sorting_1\">" + ((rs.getString("NICHO") == null) ?  "N/A" : "<a href=\"BoletosPerdidosNichos?idsorteo="+sesion.pkSorteo+"&idsector="+ rs.getString("PK_SECTOR")+"&idnicho="+rs.getString("PK_NICHO")+  "\">"+StringEscapeUtils.escapeHtml4(rs.getString("NICHO"))+"</a>")
						+ "</td>";*/
				
				html += "<td class=\"sorting_1\">" + ((rs.getString("COLABORADOR") == null) ?  "N/A" : "<a href=\"BoletosPerdidosColaborador?idsorteo="+sesion.pkSorteo+"&idsector="+ rs.getString("PK_SECTOR")+"&idnicho="+rs.getString("PK_NICHO")+ "&idcolaborador="+rs.getString("PK_COLABORADOR")+ " \">"+StringEscapeUtils.escapeHtml4(rs.getString("COLABORADOR"))+"</a>")
						+ "</td>";
