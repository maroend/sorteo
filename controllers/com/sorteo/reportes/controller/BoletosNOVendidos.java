...
import com.sorteo.reportes.model.mBoletosNOVendidos;

/**
 * Servlet implementation class BoletosNOVendidos
 */
@WebServlet("/BoletosNOVendidos")
public class BoletosNOVendidos extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request,
		...
		SesionDatos sesion;
		mBoletosNOVendidos model = new mBoletosNOVendidos();
		...
		case "listsorteos":
			...
			menu = vista.initMenu(0, false, 19, 21, sesion);
		...
		default:
			...
			fullPath = "/WEB-INF/views/reportes/boletosnovendidos.html";
	...
	protected void SorteoPredeterminado(HttpServletRequest request,
			...
			mBoletosNOVendidos model, SesionDatos sesion)
			throws ServletException, IOException {
		...
		if (idsorteo != 0 && idsector == 0 && idnicho == 0) {
			response.sendRedirect("BoletosNOVendidos?idsorteo=" + idsorteo);
		} else if (idsorteo != 0 && idsector != 0 && idnicho == 0) {
			response.sendRedirect("BoletosNOVendidosSectores?idsorteo=" + idsorteo + "&idsector=" + idsector);
		} else if (idsorteo != 0 && idsector != 0 && idnicho != 0) {
			response.sendRedirect("BoletosNOVendidosNichos?idsorteo=" + idsorteo + "&idsector=" + idsector + "&idnicho=" + idnicho);
		} else {
			response.sendRedirect("BoletosNOVendidos?view=listsorteos");
		}
	}

	protected String getBoletosTalonarios(mBoletosNOVendidos model,
			SesionDatos sesion) {
	...
	protected String getBoletosTalonariosNicho(int idsorteo, int idsector,
			int idnicho, mBoletosNOVendidos model) {
	...
	protected String GetComprador(int idsorteo, String boleto,
			String talonario, mBoletosNOVendidos model) {
	...
	protected void Buscar(HttpServletRequest request,
		...
		String HTML = CrearTabla(numeroregistros,
				model.paginacion(pg, show, search, sesion.pkSorteo), columnas,
				campos, pg, show, sesion);
		PrintWriter out = response.getWriter();
		out.println(HTML);
	...
	protected String CrearTabla(int numreg, ResultSet rs, String[] columnas,
		...
		try {
			...
			if (Integer.valueOf(numreg) > 0) {
				...
				while (rs.next()) {
					...
					html += "<td class=\"sorting_1\">"
							+ ((rs.getString("SECTOR") == null) ? "N/A"
									: "<a href=\"BoletosNOVendidosSectores?idsorteo="
											+ sesion.pkSorteo
											+ "&idsector="
											+ rs.getString("PK_SECTOR")
											+ "\">"
				...
				html += "<h1>No existen Boletos(No Vendidos) </h1>";
				html += "</div>";
				html += "</td></tr>";
			}
