...
import com.sorteo.reportes.model.mBoletosRobadosColaborador;

/**
 * Servlet implementation class BoletosRobadosColaborador
 */
@WebServlet("/BoletosRobadosColaborador")
public class BoletosRobadosColaborador extends HttpServlet {
     ...
     */
    public BoletosRobadosColaborador() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		SesionDatos sesion;
		mBoletosRobadosColaborador model = new mBoletosRobadosColaborador();
		...
		case "getBoletosTalonariosNicho":
			...
			/*
			 * idsorteo = sesion.pkSorteo; idsector = sesion.pkSector; idnicho =
			 * sesion.pkNicho; HTML =
		...
		case "getBoletosTalonariosColaborador":
			...
			int idcolaborador = sesion.pkColaborador;

			HTML = getBoletosTalonariosColaborador(idsorteo, idsector, idnicho,
					idcolaborador, model);
			break;
		...
		default:
			...
			idcolaborador = Integer.valueOf(request.getParameter("idcolaborador"));

			fullPath = "/WEB-INF/views/reportes/boletosrobadoscolaborador.html";
			menu = vista.initMenu(0, false, 19, sesion.misSorteos == 0 ? 1033 : 19, sesion);
			notificaciones = vista.initNotificaciones(context, sesion.nombreCompleto);
			infouser = vista.initInfoUser(context, sesion.nombreCompleto, sesion.role);
			HTML = vista.CrearVista(context, fullPath, menu, notificaciones, infouser);
	...
	protected void SorteoPredeterminado(HttpServletRequest request,
			...
			mBoletosRobadosColaborador model, SesionDatos sesion)
	...
	protected String getBoletosTalonarios(mBoletosRobadosColaborador model, SesionDatos sesion){
	...
	protected String getBoletosTalonariosSector(int idsorteo, int idsector,
			mBoletosRobadosColaborador model) {
	...
	protected String getBoletosTalonariosNicho(int idsorteo, int idsector,
			int idnicho, mBoletosRobadosColaborador model) {
	...
	protected String GetComprador(int idsorteo, String boleto,
			String talonario, mBoletosRobadosColaborador model) {
	...
	protected void Buscar(HttpServletRequest request,
		...
		int numeroregistros = model.contar(search,sesion.pkSorteo,sesion.pkSector,sesion.pkNicho,sesion.pkColaborador);

		
		String HTML = CrearTabla(numeroregistros, model.paginacion(pg, show, search,sesion.pkSorteo,sesion.pkSector,sesion.pkNicho,sesion.pkColaborador), columnas, campos, pg, show, sesion);
		PrintWriter out = response.getWriter();
		out.println(HTML);
	...
	protected String CrearTabla(int numreg, ResultSet rs, String[] columnas,
		...
		if (Integer.valueOf(numreg) > 0) {		
			...
			while (rs.next()) {
				...
				html += "<h1>No existen Boletos Perdidos en el Colaborador </h1>";		
				html += "</div>";
				html += "</td></tr>";
		   }		
