...
import com.sorteo.reportes.model.mBoletosRobadosNichos;

/**
 * Servlet implementation class BoletosRobadosNichos
 */
@WebServlet("/BoletosRobadosNichos")
public class BoletosRobadosNichos extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		SesionDatos sesion;
		mBoletosRobadosNichos model = new mBoletosRobadosNichos();
		...
		if (!sesion.permisos.havePermiso(20101)) {
			view = "error";
		}
		...
		if (view == null) {
			...
			if (request.getParameter("idsorteo") != null) {
				System.out.println("entra sector en default");
				// fullPath = "";
				view = "";

			} else {
				view = "ListaSorteos";
				System.out.println("entra ListaSorteos sector");
			}
		}
		...
		case "getBoletosTalonarios":
			...
			// HTML = getBoletosTalonarios();
			break;

		case "getBoletosTalonariosSector":

			/*
			 * int idsorteo = sesion.pkSorteo; int idsector = sesion.pkSector;
			 * 
		...
		case "getBoletosTalonariosColaborador":
			 ...
			 * idnicho = Integer.valueOf(request.getParameter("nicho")); int
			 * colaborador =
			 * Integer.valueOf(request.getParameter("colaborador")); HTML =
			 * getBoletosTalonariosNicho(idsorteo,idsector,idnicho);
			 */
			break;
		...
		case "GetComprador":
			/*
			 * idsorteo = Integer.valueOf(request.getParameter("sorteo"));
			 * String folio = request.getParameter("boleto"); String talonario =
			 * request.getParameter("talonario"); HTML =
			 * GetComprador(idsorteo,folio,talonario);
			 */
			break;
		...
		case "GetIncidenciaBoleto":
			...
			/*
			 * idsorteo = Integer.valueOf(request.getParameter("sorteo")); folio
			 * = request.getParameter("boleto"); talonario =
			 * request.getParameter("talonario");
			 * 
			 * HTML = GetIncidenciaBoleto(idsorteo,folio,talonario);
			 */
			break;
		...
		case "BuscarMontoAbonoTalonario":
			...
			/*
			 * idsorteo = Integer.valueOf(request.getParameter("sorteo"));
			 * talonario = request.getParameter("talonario"); HTML =
			 * BuscarMontoAbonoTalonario(idsorteo,talonario); break;
			 * 
			 * case "BuscarBoletosTalonarios": idsorteo =
			 * Integer.valueOf(request.getParameter("sorteo")); talonario =
			 * request.getParameter("talonario"); HTML =
			 * BuscarBoletosTalonarios(idsorteo,talonario);
			 */
			break;
		...
		case "error":
			...
			menu = vista.initMenu(0, false, 19, sesion.misSorteos == 0 ? 1033
					: 19, sesion);
		...
		default:
			...
			fullPath = "/WEB-INF/views/reportes/boletosrobadosnichos.html";
			menu = vista.initMenu(0, false, 19, sesion.misSorteos == 0 ? 1033 : 19, sesion);
	...
	protected void SorteoPredeterminado(HttpServletRequest request,
			...
			mBoletosRobadosNichos model, SesionDatos sesion)
		...
		if(idsorteo!=0 && idsector==0 && idnicho==0){
			response.sendRedirect("BoletosRobados?idsorteo="+idsorteo);	
		}else if(idsorteo!=0  && idsector!=0 && idnicho==0){
			response.sendRedirect("BoletosRobadosSectores?idsorteo="+idsorteo+"&idsector="+idsector);
		}else if(idsorteo!=0  && idsector!=0 && idnicho!=0){
			response.sendRedirect("BoletosRobadosNichos?idsorteo="+idsorteo+"&idsector="+idsector+"&idnicho="+idnicho);
		}
	}

	protected String getBoletosTalonarios(mBoletosRobadosNichos model,
			SesionDatos sesion) {
	...
	protected String getBoletosTalonariosSector(int idsorteo, int idsector, mBoletosRobadosNichos model) {
	...
	protected String getBoletosTalonariosNicho(int idsorteo, int idsector,
			int idnicho, mBoletosRobadosNichos model) {
	...
	protected String getBoletosTalonariosColaborador(int idsorteo,
			int idsector, int idnicho, int idcolaborador,
			mBoletosRobadosNichos model) {
	...
	protected String GetComprador(int idsorteo, String boleto,
			String talonario, mBoletosRobadosNichos model) {
	...
	protected void Buscar(HttpServletRequest request,
			...
			mBoletosRobadosNichos model, SesionDatos sesion) throws ServletException, IOException {
		...
		int numeroregistros = model.contar(search,sesion.pkSorteo,sesion.pkSector,sesion.pkNicho);

		String HTML = CrearTabla(numeroregistros, model.paginacion(pg, show, search,sesion.pkSorteo,sesion.pkSector,sesion.pkNicho), columnas, campos, pg, show, sesion);
		PrintWriter out = response.getWriter();
		out.println(HTML);
	}
	...
	protected String CrearTabla(int numreg, ResultSet rs, String[] columnas,
		...
		if (Integer.valueOf(numreg) > 0) {			
			...
			while (rs.next()) {
				...
				/*html += "<td class=\"sorting_1\">" + ((rs.getString("SECTOR") == null) ?  "N/A" : "<a href=\"BoletosRobadosSectores?idsorteo="+sesion.pkSorteo+"&idsector="+ rs.getString("PK_SECTOR")+  "\">"+StringEscapeUtils.escapeHtml4(rs.getString("SECTOR"))+"</a>")
						+ "</td>";
				
				html += "<td class=\"sorting_1\">" + ((rs.getString("NICHO") == null) ?  "N/A" : "<a href=\"BoletosRobadosNichos?idsorteo="+sesion.pkSorteo+"&idsector="+ rs.getString("PK_SECTOR")+"&idnicho="+rs.getString("PK_NICHO")+  "\">"+StringEscapeUtils.escapeHtml4(rs.getString("NICHO"))+"</a>")
						+ "</td>";*/
				
				html += "<td class=\"sorting_1\">" + ((rs.getString("COLABORADOR") == null) ?  "N/A" : "<a href=\"BoletosRobadosColaborador?idsorteo="+sesion.pkSorteo+"&idsector="+ rs.getString("PK_SECTOR")+"&idnicho="+rs.getString("PK_NICHO")+ "&idcolaborador="+rs.getString("PK_COLABORADOR")+ " \">"+StringEscapeUtils.escapeHtml4(rs.getString("COLABORADOR"))+"</a>")
						+ "</td>";
				...
				html += "<h1>No existen Boletos Perdidos en el Nicho </h1>";		
				html += "</div>";
				html += "</td></tr>";
		   }		
