...
import com.sorteo.reportes.model.mBoletosNOVendidosColaborador;

/**
 * Servlet implementation class BoletosNOVendidosColaborador
 */
@WebServlet("/BoletosNOVendidosColaborador")
public class BoletosNOVendidosColaborador extends HttpServlet {
	 ...
	 */
	public BoletosNOVendidosColaborador() {
		super();
	...
	protected void doGet(HttpServletRequest request,
		...
		SesionDatos sesion;
		mBoletosNOVendidosColaborador model = new mBoletosNOVendidosColaborador();
		...
		default:
			...
			idcolaborador = Integer.valueOf(request
					.getParameter("idcolaborador"));

			fullPath = "/WEB-INF/views/reportes/boletosnovendidoscolaborador.html";
			menu = vista.initMenu(0, false, 19, sesion.misSorteos == 0 ? 21
					: 19, sesion);
			notificaciones = vista.initNotificaciones(context,
					sesion.nombreCompleto);
	...
	protected void SorteoPredeterminado(HttpServletRequest request,
			...
			mBoletosNOVendidosColaborador model, SesionDatos sesion)
	...
	protected String getBoletosTalonarios(mBoletosNOVendidosColaborador model,
			SesionDatos sesion) {
	...
	protected String getBoletosTalonariosSector(int idsorteo, int idsector,
			mBoletosNOVendidosColaborador model) {
	...
	protected String getBoletosTalonariosNicho(int idsorteo, int idsector,
			int idnicho, mBoletosNOVendidosColaborador model) {
	...
	protected String GetComprador(int idsorteo, String boleto,
			String talonario, mBoletosNOVendidosColaborador model) {
	...
	protected void Buscar(HttpServletRequest request,
		...
		String HTML = CrearTabla(numeroregistros, model.paginacion(pg, show,
				...
				sesion.pkNicho, sesion.pkColaborador), columnas,
	...
	protected String CrearTabla(int numreg, ResultSet rs, String[] columnas,
		...
		try {
			...
			if (Integer.valueOf(numreg) > 0) {
				...
				html += "<h1>No existen Boletos(No Vendidos) del Colaborador</h1>";
				html += "</div>";
				html += "</td></tr>";
			}
