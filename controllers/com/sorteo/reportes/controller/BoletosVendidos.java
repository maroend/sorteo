...
import com.sorteo.reportes.model.mBoletosVendidos;

/**
 * Servlet implementation class BoletosVendidos
 */
@WebServlet("/BoletosVendidos")
public class BoletosVendidos extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request,
		...
		SesionDatos sesion;
		mBoletosVendidos model = new mBoletosVendidos();
		...
		case "listsorteos":
			...
			menu = vista.initMenu(0, false, 19, 20, sesion);
		...
		case "getBoletosTalonariosNicho":
			...
			HTML = getBoletosTalonariosNicho(idsorteo, idsector, idnicho, model, sesion);
			break;
		...
		case "getBoletosTalonariosColaborador":
			...
			HTML = getBoletosTalonariosColaborador(idsorteo, idsector, idnicho, idcolaborador, model, sesion);
			break;
		...
		case "Grafica":
			...
			HTML = model.create_JSON_Tree(model.consultaEstadisticas(sesion,
					"Sorteo", request));
			break;
		...
		case "error":
			...
			menu = vista.initMenu(0, false, 19, sesion.misSorteos == 0 ? 20 : 19, sesion);
		...
		default:
			...
			fullPath = "/WEB-INF/views/reportes/boletosvendidos.html";
			menu = vista.initMenu(0, false, 19, sesion.misSorteos == 0 ? 20 : 19, sesion);
			notificaciones = vista.initNotificaciones(context, sesion.nombreCompleto);
			infouser = vista.initInfoUser(context, sesion.nombreCompleto, sesion.role);
			HTML = vista.CrearVista(context, fullPath, menu, notificaciones, infouser);
	...
	protected String getSorteosUsuarios(HttpServletRequest request,
		...
		int idsorteo = modelsorteos.getIdSorteo();
		int idsector = modelsorteos.getIdSector();
		int idnicho = modelsorteos.getIdNicho();
	...
	protected void SorteoPredeterminado(HttpServletRequest request,
			HttpServletResponse response, String HTML, mBoletosVendidos model, SesionDatos sesion) throws ServletException,
			IOException {
		...
		if (idsorteo != 0 && idsector == 0 && idnicho == 0) {
			...
			response.sendRedirect("BoletosVendidosXSector?idsorteo=" + idsorteo
					+ "&idsector=" + idsector);
		} else if (idsorteo != 0 && idsector != 0 && idnicho != 0) {
			response.sendRedirect("BoletosVendidos?idsorteo=" + idsorteo
			...
			response.sendRedirect("BoletosVendidos?view=listsorteos");
		}
	}

	protected String getBoletosTalonarios(mBoletosVendidos model, SesionDatos sesion) {
	...
	protected String getBoletosTalonariosSector(int idsorteo, int idsector, mBoletosVendidos model) {
	...
	protected String getBoletosTalonariosNicho(int idsorteo, int idsector,
			int idnicho, mBoletosVendidos model, SesionDatos sesion) {
	...
	protected String getBoletosTalonariosColaborador(int idsorteo,
			...
			mBoletosVendidos model, SesionDatos sesion) {
	...
	protected String GetComprador(int idsorteo, String boleto,
			String talonario, mBoletosVendidos model, SesionDatos sesion) {
		...
		try {
			if (rs.next()) {
				...
				contenido = "|" + rs.getString("ABONO") + "|"
						...
						+ rs.getString("CORREO") + "|" + rs.getString("CALLE")
						+ "|" + rs.getString("NUMERO") + "|"
	...
	protected String GetIncidenciaBoleto(int idsorteo, String boleto,
			String talonario, mBoletosVendidos model) {
	...
	protected String BuscarMontoAbonoTalonario(int idsorteo, String talonario, mBoletosVendidos model) {
	...
	protected String BuscarBoletosTalonarios(int idsorteo, String talonario, mBoletosVendidos model) {
		String contenido = "";
		ResultSet rs = null;
		String estado = "";
	...
	protected void Buscar(HttpServletRequest request,
			HttpServletResponse response, int pg, int show, String search, mBoletosVendidos model, SesionDatos sesion)
			throws ServletException, IOException {
	...
	protected String CrearTabla(int numreg, ResultSet rs, String[] columnas,
		...
		/*
		 * html +=
		 * "<th class=\"sorting\" tabindex=\"0\" aria-controls=\"data-table\" rowspan=\"1\" colspan=\"1\" style=\"width: 115px;\" >Sector</th>"
		 * ; html +=
		 * "<th class=\"sorting\" tabindex=\"0\" aria-controls=\"data-table\" rowspan=\"1\" colspan=\"1\" style=\"width: 115px;\" >Nicho</th>"
		 * ; html +=
		 * "<th class=\"sorting\" tabindex=\"0\" aria-controls=\"data-table\" rowspan=\"1\" colspan=\"1\" style=\"width: 115px;\" >Colabolador</th>"
		 * ;
		 */
		...
		try {
			...
			if (Integer.valueOf(numreg) > 0) {
				...
				while (rs.next()) {
					...
					if ("N".equals(rs.getString("VENDIDOTALONARIO"))) {
						estadotalonario = "<span class=\"label label-inverse\" >"
								+ rs.getString("TALONARIO") + "</span>";
					}
					if ("V".equals(rs.getString("VENDIDOTALONARIO"))) {
						estadotalonario = "<span class=\"label label-success\" >"
								+ rs.getString("TALONARIO") + "</span>";
					}
					if ("P".equals(rs.getString("VENDIDOTALONARIO"))) {
				...
				html += "<h1>No existen Boletos Vendidos</h1>";
				html += "</div>";
				html += "</td></tr>";
			}
