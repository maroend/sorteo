 ...
 */
@WebServlet("/CompactoBoletosVendidos")
public class CompactoBoletosVendidos extends HttpServlet {
	 ...
	 */
	public CompactoBoletosVendidos() {
		super();
	...
	protected void Buscar(HttpServletRequest request,
			HttpServletResponse response, mStatusDeBoletos model,
			SesionDatos sesion) throws ServletException, IOException {

		StringBuilder sb = new StringBuilder();

		CrearTablaIncidencias(sb,
				model.consultaBoletosConIncidencia(sesion.pkSorteo));
		CrearTablaNoVendidos(sb,
				model.consultaBoletosNoVendidos(sesion.pkSorteo));
	...
	protected void CrearTablaIncidencias(StringBuilder sb, ResultSet rs) {
		...
		try {
			...
			while (rs.next()) {
				 ...
				 * sb.append("<div class=\"saltopagina\"></div>"); sb.append(
				 * "<table style=\"width: 100%; border-color: #FFFFFF; font-size: 10pt;\"><tbody> <tr>"
				 * ); } //
				 */

				sb.append("<td style=\"padding: 1pt;\"><strong>")
						.append(rs.getString("FOLIO")).append("</strong></td>");
				sb.append("<td>")
						.append(getNombreIncidencia(rs.getString("INCIDENCIA")))
						.append("</td>");
	...
	protected void CrearTablaNoVendidos(StringBuilder sb, ResultSet rs) {
		...
		try {
			...
			while (rs.next()) {
				...
				sb.append("<td style=\"padding: 1pt;\">")
						.append(rs.getString("FOLIO")).append("</td>");
				sb.append("<td>")
						.append(getNombreIncidencia(rs.getString("INCIDENCIA")))
						.append("</td>");
