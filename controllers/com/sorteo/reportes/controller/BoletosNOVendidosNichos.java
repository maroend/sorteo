...
import com.sorteo.reportes.model.mBoletosNOVendidosNichos;

/**
 * Servlet implementation class BoletosNOVendidosNichos
 */
@WebServlet("/BoletosNOVendidosNichos")
public class BoletosNOVendidosNichos extends HttpServlet {
	 ...
	 */
	public BoletosNOVendidosNichos() {
		super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		SesionDatos sesion;
		mBoletosNOVendidosNichos model = new mBoletosNOVendidosNichos();
		...
		if (!sesion.permisos.havePermiso(20096)){view = "error";}
		...
		case "error":
			...
			menu = vista.initMenu(0, false, 19, sesion.misSorteos==0 ? 21 : 19, sesion);
		...
		default:
			...
			fullPath = "/WEB-INF/views/reportes/boletosnovendidosnichos.html";
			menu = vista.initMenu(0, false, 19, sesion.misSorteos==0 ? 21 : 19, sesion);
			notificaciones = vista.initNotificaciones(context,
					sesion.nombreCompleto);
			infouser = vista.initInfoUser(context, sesion.nombreCompleto,sesion.role);
	...
	protected void SorteoPredeterminado(HttpServletRequest request,
			HttpServletResponse response, String HTML, mBoletosNOVendidosNichos model, SesionDatos sesion) throws ServletException,
			IOException {
		
		model.setIdUsuario(sesion.pkUsuario);
		model.getUsuarioSorteo();
		
		int sorteo = model.getIdSorteo();
		int sector = model.getIdsector();
		int nicho = model.getIdnicho();
		System.out.println(sorteo);
		
		if (sorteo !=0 && sector==0 && nicho==0){
			response.sendRedirect("BoletosNOVendidos?idsorteo="+sorteo);	
		}else if(sorteo!=0  && sector!=0 && nicho==0){
			response.sendRedirect("BoletosNOVendidosSectores?idsorteo="+sorteo+"&idsector="+sector);
		}else if(sorteo!=0  && sector!=0 && nicho!=0){
			response.sendRedirect("BoletosNOVendidosNichos?idsorteo="+sorteo+"&idsector="+sector+"&idnicho="+nicho);
		}
	}
	
	protected String getBoletosTalonarios(mBoletosNOVendidosNichos model, SesionDatos sesion){
	...
	protected String getBoletosTalonariosSector(int idsorteo, int idsector,
			mBoletosNOVendidosNichos model) {
	...
	protected String getBoletosTalonariosNicho(int idsorteo, int idsector,
			int idnicho, mBoletosNOVendidosNichos model) {
	...
	protected String GetComprador(int idsorteo, String boleto, String talonario, mBoletosNOVendidosNichos model) {
	...
	protected String GetIncidenciaBoleto(int idsorteo, String boleto, String talonario, mBoletosNOVendidosNichos model){
	...
	protected void Buscar(HttpServletRequest request,
			HttpServletResponse response, int pg, int show, String search, mBoletosNOVendidosNichos model, SesionDatos sesion)
		...
		try {
			if (Integer.valueOf(numreg) > 0)
			{
				while (rs.next()) {
					...
					html += "<td class=\"sorting_1\">" + ((rs.getString("COLABORADOR") == null) ?  "N/A" : "<a href=\"BoletosNOVendidosColaborador?idsorteo="+sesion.pkSorteo+"&idsector="+ rs.getString("PK_SECTOR")+"&idnicho="+rs.getString("PK_NICHO")+ "&idcolaborador="+rs.getString("PK_COLABORADOR")+ " \">"+StringEscapeUtils.escapeHtml4(rs.getString("COLABORADOR"))+"</a>")
							+ "</td>";
				}
			} else {
				html += "<tr> <td colspan=\"11\"> ";
				html += "<div class=\"jumbotron m-b-0 text-center\" style=\"width:100%; \">";
				html += "<h1>No existen Boletos(No Vendidos) en el Nicho</h1>";		
				html += "</div>";
				html += "</td></tr>";
			}			
		} catch (SQLException e) {
