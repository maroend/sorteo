...
import com.sorteo.reportes.model.mBoletosNOVendidosSectores;

/**
 * Servlet implementation class BoletosNOVendidosSectores
 */
@WebServlet("/BoletosNOVendidosSectores")
public class BoletosNOVendidosSectores extends HttpServlet {
	 ...
	 */
	public BoletosNOVendidosSectores() {
		super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		SesionDatos sesion;
		mBoletosNOVendidosSectores model = new mBoletosNOVendidosSectores();
		...
		if (!sesion.permisos.havePermiso(20096)) {
			view = "error";
		}
		...
		case "getBoletosTalonariosSector":
			HTML = getBoletosTalonariosSector(sesion.pkSorteo, sesion.pkSector, model);
			break;
		...
		default:
			...
			fullPath = "/WEB-INF/views/reportes/boletosnovendidossectores.html";
			menu = vista.initMenu(0, false, 19, sesion.misSorteos == 0 ? 21
					: 19, sesion);
	...
	protected void SorteoPredeterminado(HttpServletRequest request,
			HttpServletResponse response, String HTML, mBoletosNOVendidosSectores model, SesionDatos sesion) throws ServletException,
			IOException {
		...
		if (sorteo != 0 && sector == 0 && nicho == 0) {
			...
			response.sendRedirect("BoletosNOVendidosSectores?idsorteo="
					+ sorteo + "&idsector=" + sector);
		} else if (sorteo != 0 && sector != 0 && nicho != 0) {
			response.sendRedirect("BoletosNOVendidosNichos?idsorteo=" + sorteo
					+ "&idsector=" + sector + "&idnicho=" + nicho);
		}
	}

	protected String getBoletosTalonarios(mBoletosNOVendidosSectores model, SesionDatos sesion) {
	...
	protected String getBoletosTalonariosSector(int idsorteo, int idsector, mBoletosNOVendidosSectores model) {
	...
	protected String getBoletosTalonariosNicho(int idsorteo, int idsector,
			int idnicho, mBoletosNOVendidosSectores model) {
	...
	protected String getBoletosTalonariosColaborador(int idsorteo,
			...
			mBoletosNOVendidosSectores model, SesionDatos sesion) {
	...
	protected String GetComprador(int idsorteo, String boleto, String talonario, mBoletosNOVendidosSectores model) {
	...
	protected String GetIncidenciaBoleto(int idsorteo, String boleto,
			String talonario, mBoletosNOVendidosSectores model) {
	...
	protected String BuscarMontoAbonoTalonario(int idsorteo, String talonario, mBoletosNOVendidosSectores model) {
	...
	protected String BuscarBoletosTalonarios(int idsorteo, String talonario, mBoletosNOVendidosSectores model) {
		String contenido = "";
		ResultSet rs = null;
		String estado = "";
	...
	protected void Buscar(HttpServletRequest request,
		...
		String HTML = CrearTabla(numeroregistros, model.paginacion(pg, show,
				search, sesion.pkSorteo, sesion.pkSector),
			...
			if (Integer.valueOf(numreg) > 0) {
				...
				while (rs.next()) {
					...
					html += "<td class=\"sorting_1\"><input  id="
							+ rs.getString("PK1") + "  type=\"checkbox\" /></td>";
					...
					html += "<td class=\"sorting_1\">"
							+ ((rs.getString("NICHO") == null) ? "N/A"
									: "<a href=\"BoletosNOVendidosNichos?idsorteo="
											+ sesion.pkSorteo
											+ "&idsector="
											+ rs.getString("PK_SECTOR")
											+ "&idnicho="
					...
					html += "<td class=\"sorting_1\">"
							+ ((rs.getString("COLABORADOR") == null) ? "N/A"
									: "<a href=\"BoletosNOVendidosColaborador?idsorteo="
											+ sesion.pkSorteo
											+ "&idsector="
											+ rs.getString("PK_SECTOR")
											+ "&idnicho="
				...
				html += "<h1>No existen Boletos(No Vendidos) en el Sector</h1>";
				html += "</div>";
				html += "</td></tr>";
			}
