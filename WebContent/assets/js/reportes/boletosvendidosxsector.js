...
var handleListar = function(){
    ...
    /**/
    /////////////////////TALONARIOS Y BOLETOS INICIAL (Sector) ////////////
    var reporteTalonariosInicial = function(){
		var idsorteo = $('#input_sorteo_id').val();
		var idsector = $('#input_sector_id').val();
		$.ajax({ 
			type: "GET",  
			cache: false,
			url: "BoletosVendidosXSector",
   ...
   ////////////////////END BOLETOS Y TALONARIOS SORTEO///////////// 
	...
	var reporteTalonariosSector = function(idsorteo,idsector){
 		$.ajax({
			type: "GET",  
			cache: false,
			url: "BoletosVendidos",
...
function consultaBoletosXSector() {
	...
	$.ajax(
	{
		...
		success: function(msg)
		{
			// [[[65],[0],[9],[2]],[["Universidad Anáhuac México Norte"]]] 
			var info = jQuery.parseJSON(msg);
			var data = [];
			data.push(
				{name: 'Total', y: info[0][0][0], color:'rgba(0,0,0,0.7)' }
			);
			data.push(
				{name: 'Vendidos y retornados', y: info[0][1][0], color:'rgba(248,161,63,1)' }
			);
			data.push(
				{name: 'Vendidos no retornados', y: info[0][2][0], color:'rgba(0,200,200,1)' }
			);
			data.push(
				{name: 'Parcialmente vendidos', y: info[0][3][0], color:'rgba(186,60,61,1)' }
			);
			...
			$('#grafica').highcharts(
			{
				...
				tooltip: {
					...
					pointFormat: '<span>{point.name}</span>: <b>{point.y:1.0f}</b><br/>'
				},
				series:
				[{
					name: 'Boletos',
