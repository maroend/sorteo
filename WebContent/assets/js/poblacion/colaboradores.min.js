...
function AgregarColaboradores(idusuario) {
	...
	$("#idnicho").html('');
	$('#num_referencia').html('');
	$('#referencia').val('0');    // se activa temporalmente la referencia
	modo_agregar=true;
	referenciaStyles();           // Inicializa los estilos de la referencia
...
function Guardar()
{
	...
	if ($('#referencia').val()=='0')
	{
		var num_referencia = $('#num_referencia').html();
		if(num_referencia.trim()=='')
			$('#num_referencia').html('0');
	}
...
function save(action, onsuccess)
{
	...
	var b64 = btoa(
			...
			+ "&num_referencia=" + $('#num_referencia').html()
			+ "&nombre=" + $('#nombre').val()
	...
	$.ajax({
		...
		success: function (msg) {
			...
			try{
				...
				$('#num_referencia').html(data.num_referencia);
				referenciaStyles();
...
var buscarSectores=function(f_success) {
...
var buscarNichos=function(f_success) {
...
var referencia_click=function(){
	...
	if (referencia=='1'){
		...
		var clave = $('#clave').val();
		$('#img_referencia_gif').show();
		$.ajax({
			type: "GET",
			url: "Colaboradores.do",
			data: "view=Referencia&clave=" + clave,
			success: function(msg) {
