...
jQuery.fn.getCheckboxValues = function() {
    ...
    /*
	 * 
	 * function enviar() { $('#formulario2').parsley().reset(); var arr =
	 * $("input:checked").getCheckboxValues(); $("#abono").val('');
	 * if(arr.length==0){ alert("Debe elegir un boleto"); }else{
	 * 
	 * 
	 * $("#aceptarmodal").attr("onClick",
	 * "javascript:actualizarEstatusBoletoAbono('A');return false;");
	 * $('#myModal').modal('show'); $('#idboletos').val(arr);
	 ...
	 * 
	 * 
	 * var idboletos = $("#idboletos").val(); var abono = $("#abono").val();
	 * 
	 ...
	 * message: '<img src="assets/img/load-search.gif" /><br><h2>
	 * Actualizando Estatus..</h2>' });
	 * 
	 * 
	 * 
	 * $.ajax({ type: "POST", url: "BoletosSorteosNichos", data: { estatus:
	 * estatus,idboletos: idboletos,abono: abono}, success: function(msg){
	 * 
	 * $('#idboletos').val(""); $('#myModal').modal('hide');
	 * $('div.block').unblock();
	 * 
	 * 
	 * if(msg.trim()==""){
	 ...
	 * 
	 * 
	 * $.gritter.add({title:"ACTUALIZAR ESTATUS BOLETO:",text:"Se ha actualizado
	 * con exito!"});
	 * 
	 * 
	 * 
	 * }else{ $('html, body').animate({scrollTop:0}, 10); $("#alerta").fadeIn();
	 * $("#alerta").removeClass(); $("#alerta").addClass("alert alert-error");
	 * //$("#headAlerta").html("<font size=\"3\">¡Ups! ha ocurrido un Error</font>");
	 * $("#bodyAlerta").html("<strong>No te alarmes</strong>!! al parecer no
	 * se actualizo, por favor intentalo nuevamente.");
	 * 
	 *  }
	 * 
	 * Listar.init();
	 ...
	 * if(arr.length==0){ alert("Debe elegir un boleto"); }else{
	 * 
	 * $('#idboletos').val(arr); $("input:checkbox").prop('checked', false);
	 * 
	 ...
	 * 
	 * $.ajax({ type: "POST", url: "BoletosSorteosNichos", data: { estatus:
	 * estatus,idboletos: idboletos}, success: function(msg){
	 * 
	 * $('#idboletos').val(""); $('div.block').unblock();
	 * 
	 * 
	 * if(msg.trim()==""){
	 * 
...
function reporteTalonariosSector(idsorteo, idsector) {
	...
	$.ajax({
		...
		cache : false,
		url : "BoletosSorteo",
		data : "view=getBoletosTalonariosSector&sorteo=" + idsorteo
				+ "&sector=" + idsector,
		success : function(msg) {
...
function reporteTalonariosNicho(idsorteo, idsector, idnicho) {
	...
	$.ajax({
		...
		cache : false,
		url : "BoletosSorteo",
		data : "view=getBoletosTalonariosNicho&sorteo=" + idsorteo + "&sector="
				+ idsector + "&nicho=" + idnicho,
		success : function(msg) {
...
function reporteTalonariosColaborador(idsorteo, idsector, idnicho, idcolaborador) {
	...
	$.ajax({
		...
		cache : false,
		url : "BoletosSorteo",
		data : "view=getBoletosTalonariosColaborador&sorteo=" + idsorteo
				+ "&sector=" + idsector + "&nicho=" + idnicho + "&colaborador="
				+ idcolaborador,
		success : function(msg) {
...
function BuscarIncidenciaBoleto(idsorteo, boleto, talonario) {
	...
	$.ajax({
		...
		cache : false,
		url : "BoletosSorteo",
		data : "view=GetIncidenciaBoleto&sorteo=" + idsorteo
				+ "&boleto=" + boleto + "&talonario=" + talonario,
		success : function(msg) {
...
function BuscarComprador(idsorteo,pkboleto,boleto,pktalonario,talonario,abono,monto,pksector,pknicho,pkcolaborador){
	...
	$.ajax({
		...
		error : function() { alert('Error al intentar conectar con el servidor.'); }
	});
...
function InsertarVentaComprador() {
	...
	if (abono != "" && abono != 0 && nombre != "" && apellidos != ""
		...
		if (parseInt(abono) <= parseInt(costo)) {
				...
				success : function(msg) {
					...
					$.gritter
							.add({
								...
								text : 'Se ha registrado la venta del boleto <b style="color:#DED9BF;">'
										+ ultimoFolio + '</b>'
							});
					CancelarventaBoleto();
					Listar.init();
			...
			alert("Debe ingresar una cantidad menor o igual al costo del boleto: $"
					+ costo);
		}
	} else {
				...
				$.ajax({
					...
					success : function(msg) {
						...
						$.gritter
								.add({
									title : "Venta de Boleto",
									text : 'Se ha ELIMINADO la venta del boleto <b style="color:#DED9BF;">'
											+ ultimoFolio + '</b>'
								});
						CancelarventaBoleto();
						Listar.init();
				...
				if (deleteincicencia != false) {
					...
					$.gritter
							.add({
								...
								text : 'Se ha eliminado la incidencia del BOLETO <b style="color:#DED9BF;">'
										+ ultimoFolio + '</b>'
							});
				} else {
					...
					$.gritter
							.add({
								...
								text : 'Se ha registrado una incidencia al BOLETO <b style="color:#DED9BF;">'
										+ ultimoFolio + '</b>'
							});
				}
...
function BuscarMontoAbonoTalonario(sorteo, pk_talonario, idtalonario) {
	...
	$.ajax({
		...
		cache : false,
		url : "BoletosSorteo",
		data : "view=BuscarMontoAbonoTalonario&sorteo=" + idsorteo
				+ "&talonario=" + talonario,
		success : function(msg) {
...
function BuscarBoletosTalonarios(sorteo, pk_talonario, idtalonario) {
	...
	$('#boletostalonario')
			.html(
					'buscando boletos... <img style="width:14pt;" alt="..." src="assets/img/load.gif">');
